...
- **State in `indigo_send_message()`** - a broadcast message can now carry a state. `indigo_send_message()` takes an extra property argument to which one of the predefined properties *IDLE_PROPERTY*, *OK_PROPERTY*, *BUSY_PROPERTY* or *ALERT_PROPERTY* is passed, and its state is attached to the message so a stand-alone message can convey a state (e.g. an error) to the client. (Property definitions and updates have always carried both state and message; this adds a state to `indigo_send_message()` too.)
