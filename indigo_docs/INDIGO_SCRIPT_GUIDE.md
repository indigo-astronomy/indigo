...
This has a direct, practical consequence: **a script can only see the devices, agents and properties that live on the same bus instance as the Scripting Agent that runs it**. If the Scripting Agent runs inside *indigo_server*, the script can reach every driver loaded into that server. If it runs inside a GUI application that embeds its own server, the script can reach only the devices known to that application. There is no way for a script to directly reach a different server/application; if that is needed, use a network capable client living on both buses, or exchange information through custom (shared) properties as described further down.
