...
7. **Partially done — post-migration suite and trace comparison.** Generated trace captured once (`generated_reference_trace.txt`, analysis below; every `S` line equals the original). First complete run was invalid: it ran concurrently with the trace capture and every connection failed on the global device lock (two processes with device `NexDome`) — never run two NexDome test processes at once. Sequential complete run: 34 run, 32 passed, 2 failed: `simulator_protocol` (test race: the rounded `Q` heading reaches the target shortly before the simulated motion ends; fixed in the test with a 100 ms wait, then 1/1) and `shutter_states_and_rain` (migration regression: the poll treated a BUSY shutter state that it had published itself as a request still waiting in the queue, so a rain-driven close never completed; fixed with `shutter_observed` in the `.driver`, regenerated; all 6 `shutter` cases then 6/6). `reference_trace` passed (1/1) after the fix. **Not yet done:** a complete sequential suite run after the `shutter_observed` fix.
8. **In progress — defect fixes promoted.** `--known-defects` against the generated driver after the `shutter_observed` fix: 12 run, 12 passed (all NXD reproducers fixed). **Not yet done:** set `defect` to `false` for the 12 NXD cases in `cases[]` of the test (promotion into the ordinary suite); run the complete suite once; ASan/UBSan run (`make -C indigo_test test-dome-nexdome-simulator-sanitize`); opt-in network cases (`--network`); optionally rerun the final harness against the original baseline binary.
9. **Mostly done — repository integration.** `indigo.xcodeproj/project.pbxproj`: `REFACTOR.md` and `.driver` in the `dome_nexdome` group, new `fixtures/dome_nexdome` group (`plutil -lint` OK). New `indigo_dome_nexdome.vcxproj`, `.filters`, `.user` (from the `dome_baader` template, `.driver` and `REFACTOR.md` as `None` items, `xmllint` OK) and `indigo_windows.sln` entry (CRLF preserved); no Windows build was possible. `indigo_test/Makefile`: simulator depends on `serial_motion.h` (no longer `-pthread`), test depends on both fixtures and the driver archive, targets `test-dome-nexdome-simulator`, `test-dome-nexdome-simulator-sanitize`, opt-in `test-dome-nexdome-simulator-network`. `indigo_docs/PROPERTIES.md`: renamed properties, source is the `.driver`. **Not yet done:** `MIGRATION_STATUS.md` row (planned: API 3️⃣, Windows ✅ Yes, Generator ✅ Yes, Async Queues ✅ Yes, Retested ✅ Sim, tests `49 / 0` = 34 default cases + 12 promoted reproducers + 3 opt-in network cases; Comment unchanged). `README.md` unchanged.
10. **Planned — final verification.** Generator reproducibility check (regenerate, no diff), `git diff --check`, formatting audit of `.driver`/simulator/test, `make -C indigo_test test-clean`, final test summary below.

## Handover state (2026-09-17, work interrupted to continue on another computer)

- Production files: `indigo_dome_nexdome.driver` is the source of truth; `indigo_dome_nexdome.c`, `.h`, `_main.c` are regenerated from it (last regeneration includes the `shutter_observed` fix) and build without warnings.
- Tests: `indigo_test/integration/test_dome_nexdome_simulator.c` (default build = generated driver, `X_` names, version `0x0300000A`); reproducers still marked `defect = true` (run with `--known-defects`). Fixtures `indigo_test/fixtures/dome_nexdome/original_reference_trace.txt` and `generated_reference_trace.txt` are in place.
- The original-driver baseline binary was built in a temporary scratch directory and is not in the repository. To rebuild it: take `indigo_drivers/dome_nexdome/indigo_dome_nexdome.[ch]` from commit `3b7905386` into a temporary directory and run from `indigo_test`: `clang -std=gnu11 -arch arm64 -g -O1 -DINDIGO_MACOS -Duint=unsigned -I.. -I../indigo_libs -I../build/include -I../indigo_drivers -DNEXDOME_ORIGINAL_DRIVER -w -o <tmp>/test_dome_nexdome_simulator_original integration/test_dome_nexdome_simulator.c <tmp>/indigo_dome_nexdome.c -L../build/lib -Wl,-rpath,$(pwd)/../build/lib -lindigo -lm -lpthread -lindigocat`.
- Remaining steps, in order: promote reproducers (step 8), one complete sequential run, sanitizer run, network cases, `MIGRATION_STATUS.md`, final verification (step 10), fill in the final test summary, commit.
...
Interim totals (to be completed when the remaining steps are run). Trace captures count as runs of `reference_trace`; development runs of single cases are included.

- Simulated tests, original driver: 52 run, 31 passed. Breakdown: pre-existing smoke test 1/1; first complete characterization run (including the `simulator_protocol` self-check) 34 run, 26 passed (8 failures analysed in step 3); `connect_sequence` debug run 0/1 and after the test fix 1/1; defect reproducers 12 run, 0 passed (all failed as expected); trace captures 3/3.
- Simulated tests, generated driver: 89 run, 55 passed. Breakdown: invalid concurrent complete run 34 run, 2 passed (global-lock conflict with the parallel trace capture, not driver behaviour); trace capture 1/1; sequential complete run 34 run, 32 passed; `shutter` cases after the fix 6/6; `simulator_protocol` after the test fix 1/1; defect reproducers 12/12; `reference_trace` 1/1.
