...
The final physical case passed. It exercised RAW8 and RAW16 full-frame images, 256 x 256 ROI at bins 1 and 2, configuration roundtrip, long exposure with concurrent guider commands, abort and immediate reuse, finite and sustained streaming (76 full-frame RAW8 images in approximately ten seconds), CCD/guider reconnect, driver shutdown/reinitialization and a fresh exposure. The SDK advertises RGB24 and Y8 on this camera, but `SVBSetOutputImageType()` returns success while immediate `SVBGetOutputImageType()` still reports RAW8; the same behavior was reproduced by a direct SDK-only probe. The hardware harness records those two runtime refusals as SDK-specific skips, while the fake-SDK suite continues to require both formats.
...
- **Vendor SDK working-buffer overrun:** the migrated driver initially allocated only the documented maximum three-byte RGB24 payload and the SV305Pro SDK wrote past it while internally using a 7680-byte (`1920 x 4`) row stride. The buffer now accommodates the SDK's four-byte RGB32 working stride while image delivery still uses the selected format. The fake SDK asserts the maximum buffer contract; RAW8 and RAW16 hardware images pass without a crash.
...
- Hardware harness build and SV305Pro automated workflow: 1 / 1 passed. RGB24 and Y8 were explicitly skipped after direct SDK readback reproduced their runtime rejection; RAW8, RAW16 and all other automated workflows passed.
