...
## StarGO MountSim acceptance campaign — 2026-09-23

Scope: non-interactive simulator acceptance of MountSim 2.3 model `StarGO` through the production generated LX200 driver, initially 3.0.0.63. No physical hardware testing is planned or authorized for this campaign. Earlier hardware results above are separate historical runs.

Audit: `.driver` is the source of truth; `.c`, `.h`, `_main.c` are generated. The shared portable I/O handle and master queue serve mount and guider. StarGO rejects focuser and auxiliary connections. Its mount uses LX200 GR/GD, Sr/Sd/MS/CM and motion/rate commands plus X34/X38 state, X361 home, X362 park, X370 unpark, X20/X21 guide rates. The initial source audit found MountSim's Avalon implementation omitted GD dispatch and duration-based Mg pulse dispatch, returns generic acknowledgements for park/home/unpark, and has simplified motion/status and guide-rate state. These were source-audit risks before the baseline captured the failures below. The original driver expected both motor digits to equal 1 for tracking; independent protocol research below established the defect. No generator change is planned.

Build/platform: macOS arm64 host, universal x86_64/arm64 production archive and Darwin-only acceptance executable. Existing portable suite has 93 named cases, including StarGO capability/home/park/rate coverage, malformed/fault/lifecycle scenarios. Build `make -C indigo_drivers/mount_lx200 -f ../../Makefile.drv all` passed. The baseline portable run completed 93/93; the executable must run with `indigo_test` as working directory (an initial root invocation failed to locate its relative simulator executable and was stopped).

Atomic plan:
1. **Complete:** build/run unchanged portable baseline; add StarGO acceptance cases and Darwin-only Makefile/Xcode registration. Capture original-driver properties and raw protocol traces before production changes.
2. **Complete:** run all independent acceptance cases against original model/driver, preserve failing reproducers and classify each against manufacturer protocol and an independent established implementation.
3. **Complete:** fix only established driver/model defects, version-bump any changed driver, regenerate unchanged generator outputs, add portable regressions where applicable, rebuild affected components.
4. **Complete:** rerun full StarGO scope and relevant portable suite; record pulse timing, scenario map, limits and test totals; update README Testing/TEST_SUMMARY and commit this model without pushing.

Scenario map: identity/inventory/reconnect; signed fractional SYNC and actual readback; reachable GOTO/completion/already-at-target/BUSY/abort/recovery; all manual directions/rates, reversal/simultaneous axes and actual stopped position; tracking/rates/site/guide-rate reconnect readback; park/home completion, parked rejection and abort; healthy idle/active PTY-loss recovery; guider directions/durations/zero/replacement/overlap and shared ownership. Malformed replies and deterministic injection stay in the independent portable suite. MountSim is software transport acceptance, not physical motion/pulse accuracy or Linux/Windows runtime validation.

### Found defects in this campaign

Baseline discrepancies and their disposition are recorded below as LX048–LX050 and model corrections.

### Baseline evidence and protocol classification

The unchanged portable baseline completed 93/93 on macOS arm64. Initial MountSim run completed 1/10, with the apparent identity pass exposed as stale initial OK by a stronger fresh-poll assertion: identity rerun 0/1, coordinates ALERT because `:GD#` has no dispatch. Full baseline captures are retained under `/tmp/mountsim-campaign-20260923/StarGO/baseline` and `baseline-fresh`, including serial bytes, command order and property transitions. The first unpark fails because `X370` returns `1#`; site reconnect returns the original longitude because `setLongitude` never applies it. Mg commands are ignored by the app although public completion still occurs: initial timing samples alone were not functional guide coverage.

Public protocol evidence (retrieved 2026-09-23): [INDI Avalon implementation](https://github.com/indilib/indi-3rdparty/blob/master/indi-avalon/lx200stargo.cpp), [independent MATLAB StarGO client](https://github.com/farhi/matlab-stargo/blob/master/%40stargo/stargo.m), and [public command sheet cited by MountSim's original author](https://docs.google.com/spreadsheets/d/1J-Hycie6MfPk4ZQRMJrDvBi_IJG1nwcU4JlWg0GLxbk/edit#gid=592260319). The manufacturer's download page provides user manuals but no located command specification; the sheet is not claimed to be manufacturer-authored. INDI and MATLAB independently confirm park `pB`, unpark `p0`, four-digit Mg pulses, independent guide rates, and MS success `0`. MATLAB confirms GD sexagesimal format; the sheet and INDI explicitly identify `m10` as tracking with DEC stopped. No driver change to accept the emulator's wrong park replies is appropriate.

The initial baseline completed step 1. Step 2 established MountSim defects in missing coordinate/pulse handlers, incorrect park/unpark acknowledgements, ignored longitude/rate settings and incomplete motion/status/coordinate semantics. Step 3 implemented model corrections grounded in these commands: actual home/park movement and state completion, SYNC coordinate rebasing, duration-based guiding, valid GD/site fields and GOTO/status semantics. A dedicated portable m10 reproducer preceded the driver tracking fix. No generator change is needed.

Coordinator infrastructure evidence: shared launcher holds a per-user `/tmp` flock for the entire model run and releases after normal signal cleanup. Two-process exclusion/release, bounded wait timeout and release after owner termination were verified by the coordinator. This keeps all GUI sessions serial and owned by their launcher. The launcher also exposes requested/actual model and actual version to model-specific registries; StarGO does not consume those optional variables, and its registry remained 14 cases after that plumbing change.

LX048 (reproduced, fixed in source): StarGO tracking turns OFF after a successful ON request when the next poll returns `m10`; the simulator previously concealed this with an invalid `p11`. The new portable `lx200_stargo_tracking_status_readback` failed against v63 with `m10`. The first v64 predicate also failed a strengthened `m15` regression: INDI's `getMotorStatus` documents independent RA/DEC digits, so RA digit 1 remains tracking during DEC manual motion. The final predicate reads RA digit alone. Portable model now reports valid per-axis `m` status. Test-only correction: pole readback is compared after the driver's configured current-epoch-to-J2000 conversion, rather than incorrectly demanding published J2000 DEC exactly 90.

LX049 (reproduced): aborting StarGO park leaves MOUNT_PARK BUSY and blocks subsequent motion. Portable `lx200_stargo_park_abort_settles` and MountSim `lx200_park_abort_recovery` reproduce it. The scoped abort fix cancels pending StarGO park/home handlers and settles their state after the global stop succeeds. MountSim had an independent stop defect: `stopSlew` affects manual axes only, so its Q handler now clears GOTO as well, without reporting arrival after abort.

LX050 (reproduced): reversing/stopping one manual StarGO axis uses global Q, stopping the other while its public property stays BUSY. Both independent [INDI MoveNS/MoveWE](https://github.com/indilib/indi-3rdparty/blob/master/indi-avalon/lx200stargo.cpp) and [MATLAB stop_slew direction commands](https://github.com/farhi/matlab-stargo/blob/master/%40stargo/stargo.m) use Qn/Qs/Qe/Qw, corroborating the model's existing directional-stop support. The driver now uses its existing per-direction stop path for StarGO too. Dedicated portable command regression and MountSim simultaneous-axis reversal/readback characterize this intentional change from original global-stop traces; that manual-stop change leaves other mount branches unchanged.

### Implementation and verification progress

Steps 1–3 are complete: original baseline retained, discrepancies classified, targeted model fixes built and the generated driver rebuilt at 3.0.0.64. The four new portable regressions cover m10/m15 tracking, stopped park settlement, direction-specific manual stop and a deterministic queued-park/urgent-abort race. The race failed with one X362 after abort before cancellation was added; its gate blocks only a test handler and is always released during cleanup. Registered portable count is now 97; the existing 35 physical-hardware cases and historical hardware evidence are unchanged. The final portable suite passed 97/97 after the corrections below; the complete StarGO scope also passed 14/14 against the final rebuilt driver. Focused ASan/UBSan StarGO cases and strict harness compilation passed.

MountSim changes are confined to Avalon.m and its existing audit assertion. The Avalon-local motor subclass rebases SYNC readback without altering other models, makes home/park finish at the staged pole and preserves their nontracking end state. Its GOTO speed ceiling now uses the fastest already-declared Avalon slew-table entry rather than the 10x find-rate entry; this exercises elapsed-time motion without claiming measured hardware speed. Q now stops both GOTO and manual motion. Guide commands use the shared model's duration-to-displacement helper; this is a simplified integrated guide displacement model, not an actual timed motor-current simulation. The unrelated families' model implementations are unchanged.

| StarGO acceptance case | Driver/model evidence |
| --- | --- |
| `lx200_rejects_unsupported_accessories` | Focuser/AUX connection rejected, mount retains live shared handle and fresh polling |
| `lx200_rejected_goto_recovers` | Below-horizon target rejected; next reachable target arrives by readback |
| `lx200_guider_motion_readback` | 2000 ms north pulse at 50% changes actual DEC, followed by stable fresh readback |
| `lx200_park_abort_recovery` | Running park stopped, unparked/non-BUSY state, reconnect and new SYNC |
| `lx200_identity_reconnect` | Auto-detected StarGO, property inventory, fresh coordinate poll and reconnect |
| `lx200_sync_signed_readback` | Positive/fractional/negative-zero-degree DEC; exact RA/DEC readback without MS |
| `lx200_goto_busy_abort_recovery` | Reachable and already-at-target completion; BUSY request exclusion; abort and fresh GOTO |
| `lx200_tracking_and_rates` | Sidereal/solar/lunar wire commands and tracking state after subsequent polls |
| `lx200_site_and_guide_rate_roundtrip` | Southern/western site and independent RA/DEC guide rates survive reconnect readback |
| `lx200_manual_axes_rates_stops` | Four rates/directions on wire; all four measured directions, simultaneous axes, reversal and fresh stopped-position samples |
| `lx200_park_home_and_rejection` | Actual parked/home coordinate readback, tracking off, parked motion rejection, unpark |
| `lx200_idle_transport_loss` | Healthy idle polling, PTY removal, ALERT, fresh PTY reconnect and SYNC |
| `lx200_active_transport_loss` | PTY loss during GOTO, ALERT, reconnect/abort and new SYNC |
| `lx200_guider_timing_and_shared_lifecycle` | 20/100/500 ms in all directions, three repeats after discarded warmups, idle and tracking/polling; zero/no-op, replacement, axes overlap, sibling survival, disconnect/reconnect |

Not applicable or separately covered: StarGO's driver hides UTC/host-time, home/park-position setters, PEC and alignment controls; there is no device time readback contract to assert here. INDI can expose optional StarGO accessories, but this INDIGO profile does not support them and explicitly rejects them. Electrical serial faults, physical removal, mechanical pointing, actual relay pulse timing, other firmware and Linux/Windows runtimes remain unverified. Malformed/fault replies and open rollback are covered by the portable suite, not synthetic changes to the byte-transparent MountSim relay. Driver INIT/SHUTDOWN is exercised; no dynamic-library unload claim is made.

Protocol framing and model limit: [INDI StarGO query defaults](https://github.com/indilib/indi-3rdparty/blob/master/indi-avalon/lx200stargo.h) read through `#` for MS; MATLAB likewise configures `#` as the serial terminator and decodes MS success as 0, supporting `0#`. The existing MountSim generic simple-ACK path still emits `1#` for some commands that INDI sends without reading a reply (direction, tracking and Q). This retained simplification is not claimed as full wire conformance; the INDIGO transport discards unsolicited input before the next transaction.

### Final execution evidence

The final complete frozen-binary StarGO run passed 14/14 using `python3 mountsim/run_mountsim.py --app /Users/polakovic/Development/MountSim/build/Build/Products/Debug/MountSim.app --mount StarGO --terminator 23 --output /tmp/mountsim-campaign-20260923/StarGO/final --test build/mountsim/test_mount_lx200_mountsim --timeout 180` from `indigo_test`. The configurable Makefile equivalent is `make -C indigo_test test-mount-lx200-mountsim MOUNTSIM_APP=<app> MOUNTSIM_LX200_MODEL=StarGO`. Raw serial events, property transitions, model logs, production-binary hashes and pulse samples are preserved outside test build cleanup under `/tmp/mountsim-campaign-20260923/StarGO/`. Baseline repository commits were INDIGO `183bfe9d4997ed55808caae7342d1f8cf8b1731c` and MountSim `75c84c37c8b1c8b56f4e027efc039a30daa58163`; the changes in this campaign were built on those revisions. Production binaries remained frozen throughout all 14 cases.

Pulse measurements use relay command-send time to public guider completion, with signed error = measured interval minus requested duration. Mg has no OFF command: no artificial stop edge was measured. Each workload has four directions × three durations (20/100/500 ms) × three retained samples after discarded warmups. Independent 2000 ms DEC guiding verifies actual model displacement. These are host/software timing observations, not hardware timing or a strict accuracy certification.

| Workload | n | Minimum | Mean | Median | p95 | p99 | Maximum/max absolute | Standard deviation |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Idle | 36 | 50.226 | 55.076 | 55.367 | 59.482 | 59.798 | 59.798 | 2.328 |
| Tracking with polling | 36 | 50.348 | 68.614 | 55.242 | 185.583 | 250.544 | 250.544 | 42.132 |
| Combined | 72 | 50.226 | 61.845 | 55.291 | 112.455 | 250.544 | 250.544 | 30.595 |

All table values except counts are milliseconds of signed completion error; the mean signed relative error across all requests was 130.239%, maximum absolute relative error 1252.720% (short requests magnify the fixed overhead). The acceptance bound checks completion liveness, while retaining this measured latency rather than claiming precision.

The first expanded portable run finished 95/97: the new queued-park test incorrectly restored the generic client callback instead of the existing timed observer, hiding later NYX coordinates and guider completion observations. Its callback now delegates to and restores the timed observer; its captured device pointer is atomic and reset for each run. This was a test-harness defect, not a NYX production fix. The subsequent serial 97-case rerun passed 96/97 and exposed the aGotino completion boundary described below. Focused StarGO ASan/UBSan previously passed 9/9; strict new-harness compilation passed with `-Wall -Wextra -Werror` and existing unused/sign comparison exceptions.

LX051 (reproduced, fixed): the expanded portable regression run exposed an aGotino completion race. A poll reads GR/GD while its firmware still reports frozen pre-slew coordinates, then D can report idle after the physical move completes. The driver published OK with the old coordinate snapshot. A deterministic `D / COMPLETE` simulator control finishes the actual shared motion state at exactly that boundary; the existing `lx200_agotino_holds_the_goto_until_the_slew_ends` case failed its immediate 22 h readback before the fix. The scoped aGotino status handler now rereads coordinates after a successful idle D response while `goto_issued` is still true; failure retains ALERT and recovery can retry. This does not weaken the end-position assertion or change other models' polling order. Five consecutive deterministic repetitions passed after the fix; the full 97-case portable suite and StarGO 14-case acceptance both passed using the rebuilt production driver. The original full failing event filename was inadvertently reused by focused repetitions; its captured failing command excerpt and failed assertion log remain in `agotino-boundary`, alongside the later full passing traces.

LX051 protocol attribution: the [aGotino firmware](https://github.com/mappite/aGotino/blob/main/aGotino.ino) documents frozen initial coordinates during motion (lines 354–362), returns those strings and a DEL distance bar inside the slew loop (400–408), updates coordinate strings only after slew returns (520–524), and returns an empty idle distance bar (555–557). Independently, [INDI LX200 Basic ReadScopeStatus](https://github.com/indilib/indi/blob/master/drivers/telescope/lx200basic.cpp) reads coordinates and checks target arrival before marking completion; the aGotino README lists that client as tested. Together these establish firmware-compatible simulator behavior and a driver snapshot-ordering defect. The new failure path sets `coordinate_read_failed` and ALERT; the downstream poll checks that flag before its BUSY/OK branches, so the error is not overwritten and the outstanding goto can retry its readback. This campaign did not rerun aGotino hardware.

### Final test summary — completed 2026-09-23

- MountSim 2.3 StarGO acceptance: **14 run, 14 passed**, final production driver **3.0.0.64** on macOS arm64; frozen app/driver/test binaries, final artifacts in `StarGO/final`.
- Portable LX200 simulator regression: **97 run, 97 passed**, command `build/integration/test_mount_lx200_simulator` from `indigo_test`; log `stargo-portable-verified.log`.
- Focused ASan/UBSan: **10 run, 10 passed** (nine StarGO/StarGO2 selections plus the deterministic aGotino completion case); commands `build/integration/test_mount_lx200_simulator_sanitize stargo` and `build/integration/test_mount_lx200_simulator_sanitize lx200_agotino_holds_the_goto_until_the_slew_ends`; no sanitizer findings.
- Deterministic aGotino boundary stress: **5 run, 5 passed** after the preserved failing reproducer; included again in the full suite and sanitizer selection.
- Strict C harness compilation, universal production driver build, MountSim app build, unchanged-generator reproducibility, Python syntax, Xcode registration and diff whitespace checks passed. No generator change or public property addition/removal occurred.
