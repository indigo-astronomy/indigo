...
static uint8_t ra_pwmindex;
static uint32_t ra_wormperiod;
...
static uint8_t de_pwmindex;
static uint32_t de_wormperiod;
...
static uint8_t replyindex;
static uint8_t read;
static uint32_t lastraTime;
static uint32_t lastdeTime;
...
void setupRA(uint32_t nb_teeth, uint32_t gear_ratio_num, uint32_t gear_ratio_den, uint32_t nb_steps, uint32_t nb_microsteps, uint32_t highspeed) {
...
void setupDE(uint32_t nb_teeth, uint32_t gear_ratio_num, uint32_t gear_ratio_den, uint32_t nb_steps, uint32_t nb_microsteps, uint32_t highspeed) {
  ...
  de_steps_360       = ((uint32_t)de_nb_teeth * de_gear_ratio_num * de_nb_steps * de_microsteps) / de_gear_ratio_den;
  de_steps_worm      = ((uint32_t)de_gear_ratio_num * de_nb_steps * de_microsteps) / de_gear_ratio_den;
...
static void compute_timer_ra(uint32_t wormperiod) {
  uint32_t n = (wormperiod * MUL_RA);
  n += ((wormperiod * REM_RA) / ra_steps_worm);
  ra_period = n;
}

static void compute_timer_de(uint32_t wormperiod) {
