...
| DRV-131 | Medium | `agent_guider/indigo_agent_guider.c:2059` | The inter-frame delay checks abort on each existing sleep iteration, then clears delay and follows normal guiding finalization. Four focused cases pass, including abort during 4 s/0.5 s delays, shutdown during delay, no subsequent exposure and restart. Version remains `0x0300002C` for the combined change. | Closed (fixed) |
