...
### Scoped review — Rumen's ToupTek wheel fixes (2026-09-14)

Reviewed `e6f66fbeea44330d171c6cfb6716cede2930aaff` and `e0608400bc877ed4004535ffc75d57c1d1b2a5cc` against their parents, including the shared OEM source and surrounding wheel connection/movement/calibration handlers. Source is unchanged at workspace HEAD `5dd7a9415`. No new regression found: connection completion now initializes the one-based private current/target slots from the same SDK readback used for the public property; the follow-up restores DEBUG logging. The shared driver version increases from `0x0300002c` to `0x0300002d`. No properties were added or removed.

Validation: the ToupTek fake-SDK test executable built for macOS arm64/x86_64; both wheel cases passed on arm64 (`INDIGO_TEST_FILTER=Wheel`), covering slot changes, command failure, wheel models, calibration and limits. Physical hardware, separate OEM binaries and the complete camera/focuser suite were not run for this scoped review. The related bus fix and reported Scripting Agent crash are recorded in `indigo_libs/REVIEW.md`. Whole-folder review baseline unchanged.
...
| DRV-052 | Medium | `rotator_asi/indigo_rotator_asi.c:732` | Potential macOS HIDAPI hot-plug enumeration failure: bundled libCAA.a defines HIDAPI with a persistent IOHIDManager bound to the initializing thread run loop. Hot-plug SDK enumeration runs in fresh timer callback threads, so later enumeration can pump a different loop and reuse stale manager state. Binary structure and thread dispatch verified statically; reproduce with hardware before claiming observed failure. | Closed (re-verified 2026-09-19, no longer applicable). The finding rests on the dispatch described in the 2026-09-07 scan: hot-plug callbacks going through `indigo_set_timer`, which creates a new callback thread per firing, so a later SDK enumeration could pump a different run loop than the one the persistent IOHIDManager was scheduled on. The driver no longer does that. Plug, unplug and the connection handler are all queued onto a single `driver_queue` (`indigo_queue_add`/`indigo_queue_add_with_data`), and `indigo_queue_create()` starts one long-lived thread, so every call that can create or use the manager runs on the same thread whichever one runs first. Confirmed at the library level too: in the x86_64 slice of `libCAA.a`, `_hid_mgr` is referenced only by `hid_init`, `hid_enumerate`, `hid_open_path` and `hid_exit`, reachable only from `CAAGetNum` -> `hid_enumerate` and `CAAOpen` -> `CCAA::CCAA()`/`CCAA::open()`. `CAAGetSDKVersion` and `CAAGetProductIDs`, the two calls that do run on other threads (the libusb hot-plug callback and driver INIT), do not touch HIDAPI at all. In the driver those entry points appear only at `indigo_rotator_asi.c:106`/`154` (via `asi_open` from `rotator_connection_handler`) and `666`/`683` (`process_plug_event_handler`), both on `driver_queue`. Caveat: x86_64 slice only, and this is still static analysis - it shows the precondition is gone, not that the vendor library is correct.) |
| DRV-053 | Medium | `guider_asi/indigo_guider_asi.c:473` | Potential macOS HIDAPI hot-plug enumeration failure: bundled libUSB2ST4Conv.a defines HIDAPI with a persistent IOHIDManager bound to the initializing thread run loop. Hot-plug SDK enumeration runs in fresh timer callback threads, so later enumeration can pump a different loop and reuse stale manager state. Binary structure and thread dispatch verified statically; reproduce with hardware before claiming observed failure. | Closed (re-verified 2026-09-19, no longer applicable). The finding rests on the dispatch described in the 2026-09-07 scan: hot-plug callbacks going through `indigo_set_timer`, which creates a new callback thread per firing, so a later SDK enumeration could pump a different run loop than the one the persistent IOHIDManager was scheduled on. The driver no longer does that. Plug, unplug and the connection handler are all queued onto a single `driver_queue` (`indigo_queue_add`/`indigo_queue_add_with_data`), and `indigo_queue_create()` starts one long-lived thread, so every call that can create or use the manager runs on the same thread whichever one runs first. `USB2ST4GetNum`/`USB2ST4GetID`/`USB2ST4Open` are reached only from `guider_connection_handler`, `process_plug_event_handler` and `process_unplug_event_handler`, all queued onto `driver_queue`. The vendor archive was not re-disassembled; the dispatch evidence alone removes the precondition.) |
| DRV-054 | Medium | `wheel_playerone/indigo_wheel_playerone.c:589` | Potential macOS HIDAPI hot-plug enumeration failure: bundled libPlayerOnePW.dylib defines HIDAPI with a persistent IOHIDManager bound to the initializing thread run loop. Hot-plug SDK enumeration runs in fresh timer callback threads, so later enumeration can pump a different loop and reuse stale manager state. Binary structure and thread dispatch verified statically; reproduce with hardware before claiming observed failure. | Closed (re-verified 2026-09-19, no longer applicable). The finding rests on the dispatch described in the 2026-09-07 scan: hot-plug callbacks going through `indigo_set_timer`, which creates a new callback thread per firing, so a later SDK enumeration could pump a different run loop than the one the persistent IOHIDManager was scheduled on. The driver no longer does that. Plug, unplug and the connection handler are all queued onto a single `driver_queue` (`indigo_queue_add`/`indigo_queue_add_with_data`), and `indigo_queue_create()` starts one long-lived thread, so every call that can create or use the manager runs on the same thread whichever one runs first. `POAGetPWCount`/`POAGetPWProperties`/`POAOpenPW` are reached only from `wheel_connection_handler`, `process_plug_event_handler` and `process_unplug_event_handler`, all queued onto `driver_queue`. The vendor dylib was not re-disassembled; the dispatch evidence alone removes the precondition.) |
...
| DRV-085 | Medium | `aux_cloudwatcher/indigo_aux_cloudwatcher.c:719` | Low-resolution humidity uses the temperature coefficient and reports negative humidity for valid sensor data. | Closed (fixed in `8bb232c44`, driver version `0x0200000A`; the record was left open by mistake) |
| DRV-086 | High | `aux_cloudwatcher/indigo_aux_cloudwatcher.c:376` | A serial timeout before the first reply byte writes response[-1]. | Closed (fixed in `8bb232c44`, driver version `0x0200000A`; the record was left open by mistake) |
| DRV-087 | High | `aux_dragonfly/shared/dragonfly_shared.c:121` | A full-size UDP reply writes the terminator one byte past the response buffer. | Closed (fixed 2026-09-19: the UDP branch read `LUNATICO_CMD_LEN` bytes regardless of the caller's `max`, then wrote the terminator at `response[index]`, one past a `char[LUNATICO_CMD_LEN]` buffer on a full-size datagram. It now reads `max`, and the four call sites pass `sizeof(response) - 1`, matching the `max`-is-payload-capacity contract and the identical fix made for `focuser_lunatico` in DRV-193. Versions: aux_dragonfly `0x02000005`, dome_dragonfly `0x02000006`. Not covered: `PRIVATE_DATA->udp` is only set for a `udp://` port and the bundled simulator is a PTY, so the UDP branch has no test; the serial suite and its ASan variant both pass.) |
...
| DRV-096 | High | `focuser_lunatico/shared/lunatico_shared.c:336` | A full-size serial/UDP reply overflows the response terminator; reproduced through rotator_lunatico with ASan. | Closed (duplicate of DRV-193, fixed in `8bb232c44`: the UDP branch now reads `max` and all three call sites pass `sizeof(response) - 1`; the record was left open by mistake) |
| DRV-097 | Medium | `focuser_lunatico/shared/lunatico_shared.c:1543` | Rejected rotator GOTO never publishes ALERT and idle polling reports OK instead. | Closed (duplicate of DRV-194, fixed in `8bb232c44`: a failed rotator GOTO publishes ALERT and does not start the poll; the record was left open by mistake) |
...
| DRV-121 | Low | `ccd_asi/indigo_ccd_asi.c:590` | Streaming countdown decrements the fractional exposure value by one per sleep and publishes it without rounding (e.g. 3.5 s produces 2.5 and 1.5 s). Derive the display from a monotonic deadline and publish whole seconds rounded upward, independently of SDK acquisition timing. Static finding; hardware reproduction pending. | Closed (already implemented; verified 2026-09-19) The driver no longer runs its own countdown: the exposure display is produced by `countdown_task_callback()` in `indigo_ccd_driver.c`, which derives the remainder from the monotonic `countdown_endtime`, publishes `ceil(time_left)` and only when the whole second changes. `ccd_asi` sets `exposure_end = indigo_monotonic_time() + exposure_duration` and the streaming display uses `ceil(fmax(0, exposure_end - now))`; nothing decrements the requested duration any more. `frame_types_and_fractional_countdown` in `test_ccd_asi_sdk.c` now also asserts that every published value below the requested 2.5 s is a whole number, which the old fractional decrement would have failed. |
| DRV-122 | Low | `ccd_svb/indigo_ccd_svb.c:582` | Streaming countdown subtracts integer wall-clock seconds from the exact exposure target and publishes the fractional remainder without rounding. `time(NULL)` also gives coarse elapsed time and is affected by wall-clock adjustments. Use a monotonic deadline and round only the displayed remainder upward, preserving the requested SDK exposure duration. Static finding; hardware reproduction pending. | Closed (already implemented; verified 2026-09-19) The driver no longer runs its own countdown: the exposure display is produced by `countdown_task_callback()` in `indigo_ccd_driver.c`, which derives the remainder from the monotonic `countdown_endtime`, publishes `ceil(time_left)` and only when the whole second changes. `time(NULL)` is gone from `ccd_svb`; the deadline is `indigo_monotonic_time()` based and the streaming display uses `ceil(fmax(0, exposure_end - now))`. The requested SDK exposure is unchanged, which the same test checks (`SVB_EXPOSURE` still 2500000). `frame_types_and_fractional_countdown` in `test_ccd_svb_sdk.c` now asserts the published countdown is integral. |
| DRV-123 | Low | `ccd_ptp/indigo_ptp_canon.c:1637` | The legacy Canon BulbStart/BulbEnd branch publishes a fractional countdown by decrementing `CCD_EXPOSURE_ITEM->number.value`; the same value controls the final sleep before closing the shutter. Separate the exact monotonic shutter deadline from the rounded display before changing countdown publication, otherwise rounding can lengthen the exposure. The separate `ptp_blob_exposure_timer()` already rounds its display. Static finding; hardware reproduction pending. | Closed (already implemented; verified 2026-09-19) The legacy Canon branch no longer publishes its own countdown. `ptp_capture_start()` keeps the exact shutter deadline separately in `PRIVATE_DATA->shutter_deadline` (`indigo_monotonic_time() + CCD_EXPOSURE_ITEM->number.target + delay`) and leaves the display to `indigo_ccd_resume_countdown()`, so rounding the display can no longer lengthen the exposure. Covered by `canon_countdown_with_occupied_queue` in `test_ccd_ptp_sdk.c`, which waits for the published countdown to reach the integer 1. |
...
| DRV-130 | Medium | `agent_guider/indigo_agent_guider.c:2684` | RA-only dithering now preserves the signed requested magnitude by projecting it onto the image axes with `cos(angle)` and `sin(angle)`. `integration/test_agent_guider.c` verifies the magnitude in `dither RA projection preserves magnitude`. | Closed (fixed) |
...
| DRV-162 | High | `agent_config/indigo_agent_config.c:505` (relevant lines 505–523) | **Serialization failure still publishes SAVE/OK.** All indigo_save_property return values are discarded. Injecting INDIGO_FAILED at that API after the output file opens leaves an empty saved file and reports SAVE/OK. Existing configurations are opened for overwrite, so real write failures can destroy a previously valid snapshot. Check serialization results and make replacement transactional. Regression cases: `save_write_failure`. | Closed (fixed) |
| DRV-163 | Medium | `agent_config/indigo_agent_config.c:542` (relevant lines 542–550, 172–204) | **An all-false LOAD request deselects the live configuration.** Even with no selected configuration, LOAD schedules load_configuration, deselects the active camera and related agents, and clears LAST before reporting success. Treat an empty selection as a no-op or explicit error before mutating the live setup. Regression cases: `empty_load`. | Closed (fixed) |
| DRV-164 | High | `agent_config/indigo_agent_config.c:481` (relevant lines 481–504) | **SAVE allows a name to escape the configuration directory.** SAVE only replaces whitespace and passes ../outside to indigo_open_config_file, which constructs a path relative to the configuration directory. The isolated request succeeds and writes outside .indigo, within the test-owned temporary root. REMOVE already rejects forward slashes. Apply consistent basename validation before opening any output; no real user configuration was touched. Regression cases: `save_path_separator`. | Closed (fixed) |
| DRV-165 | Low | `agent_config/indigo_agent_config.c:744` (relevant lines 744, 757, 790) | **Update and delete match a different filter prefix than define.** Definitions require the seven-character FILTER_ prefix, but update/delete compare only six characters. A changed FILTERX_CCD_LIST update creates a configuration mirror even though its definition was ignored. Match FILTER_ consistently. Regression cases: `filter_prefix_consistency`. | Closed (fixed) |
| DRV-166 | Medium | `agent_config/indigo_agent_config.c:104` (relevant lines 104–116, 475–480) | **SETUP reports OK when its automatic persistence fails.** A non-writable configuration destination makes save_config set hidden CONFIG/ALERT, but the visible SETUP property still publishes OK. The user cannot tell that the setting was not persisted from its result. Propagate persistence failure to the visible SETUP acknowledgement. Regression cases: `setup_save_failure`. | Closed (fixed) |
| DRV-167 | High | `agent_config/indigo_agent_config.c:368` (relevant lines 368–387, 731) | **A later successful filter update hides an earlier selection failure.** The mirror has one state, overwritten by each constituent filter property. A rejected camera selection sets ALERT, but successful related-agent restoration then sets that same mirror OK; LOAD reports success with no camera selected. The isolated camera-only rejection test correctly reports ALERT. Track per-property restoration results rather than the last update for the entire agent. Regression cases: `selection_alert_masked`. | Closed (fixed) |
| DRV-168 | Medium | `agent_config/indigo_agent_config.c:290` (relevant lines 290–309) | **Unload-unused policy does not disable drivers absent from the saved list.** After saving, introduce a fourth enabled server driver and enable UNLOAD_UNUSED_DRIVERS. Restore only sends the three saved items; the new driver remains enabled although it is unused by the configuration. Build the restore command against the current complete driver inventory when unloading is requested. Regression cases: `driver_unload_new`. | Closed (fixed) |
| DRV-169 | High | `agent_scripting/indigo_agent_scripting.c:309`, `agent_scripting/indigo_agent_scripting.c:337` | BLOB bindings dereferenced a null native item when JavaScript supplied an object without the private `reference`, allowing a script to crash the agent process. Reject missing references before reading BLOB fields. | Closed (fixed and verified, 2026-09-14) |
| DRV-170 | Medium | `agent_scripting/indigo_agent_scripting.c:401` | Script-originated text-property changes copied values through a fixed-size buffer and truncated long text supported by raw INDIGO text items. Use the dynamic text-item setter. | Closed (fixed and verified, 2026-09-14) |
| DRV-171 | Medium | `agent_scripting/indigo_agent_scripting.c:974` | `indigo_utc_to_delay()` ignored parse failure and calculated from a zeroed time structure. Return a JavaScript error for invalid UTC input. | Closed (fixed and verified, 2026-09-14) |
| DRV-172 | High | `agent_scripting/indigo_agent_scripting.c:1060`, `agent_scripting/indigo_agent_scripting.c:1264` | On-load hooks ran before asynchronous CONFIG restoration finished, so saved hooks could be skipped or use incomplete script data. Delay execution until CONFIG finishes and propagate restoration/hook failure. | Closed (fixed and verified, 2026-09-14) |
| DRV-173 | Medium | `agent_scripting/indigo_agent_scripting.c:1288` | Repeated configuration loading treated an existing saved script as a duplicate user add and recursively attempted configuration saves during restore. Make CONFIG_READER restoration idempotent and suppress restore-time saves. | Closed (fixed and verified, 2026-09-14) |
| DRV-174 | Medium | `agent_scripting/indigo_agent_scripting.c:1377` | Script deletion neither published a successful acknowledgement nor reported an unknown name, leaving clients without a terminal result. Always publish OK or ALERT and clear the request. | Closed (fixed and verified, 2026-09-14) |
| DRV-175 | Medium | `agent_scripting/indigo_agent_scripting.c:1434` | Saved scripts could be renamed to an empty or duplicate name, making name-based execution/deletion ambiguous. Reject invalid rename requests before copying values. | Closed (fixed and verified, 2026-09-14) |
| DRV-176 | Medium | `agent_scripting/indigo_agent_scripting.c:1065`, `agent_scripting/indigo_agent_scripting.c:1512` | Failed on-load and on-unload scripts were reported as OK. Aggregate hook results, continue selected unload hooks and publish ALERT when any execution fails. | Closed (fixed and verified, 2026-09-14) |
| DRV-177 | Medium | `agent_scripting/indigo_agent_scripting.c:1606` | Whole-device delete events passed an empty string to `boot.js`, whose cache-removal branch expects JavaScript null, so detached devices remained cached. Map an empty property name to null. | Closed (fixed and verified, 2026-09-14) |
| DRV-178 | Critical | `agent_scripting/indigo_agent_scripting.c:1104` | The declared 128-script capacity allocated too few switch items; adding the 127th/128th script wrote beyond the heap buffers for execute/load/unload properties. Allocate 128 execution items and 129 hook items. | Closed (fixed and verified with ASan, 2026-09-14) |
| DRV-179 | High | `agent_scripting/library/Sequencer.js:23` | Public sequence helpers embedded text arguments directly into executable JavaScript microsteps, so quotes/backslashes broke valid inputs and allowed injected operations. Serialize every textual argument with `JSON.stringify()`. | Closed (fixed and verified, 2026-09-14) |
| DRV-180 | High | `agent_scripting/library/Sequencer.js:991` | An exception in a generated microstep escaped the timer callback and left the sequence permanently Busy, bypassing abort/continue/recover policies. Catch execution errors, route them through `failure()`, and handle missing devices in all property setters. | Closed (fixed and verified, 2026-09-14) |
| DRV-181 | Medium | `agent_scripting/library/Sequencer.js:788` | Sequencer action properties requested noncanonical `OneOfMany`, which the native binding interpreted as AnyOfMany. Use the canonical `ONE_OF_MANY` rule. | Closed (fixed and verified, 2026-09-14) |
| DRV-182 | High | `agent_guider/indigo_agent_guider.c:2250` | Frame metadata can arrive before binning metadata; dividing by zero and converting infinity to `int` caused undefined behavior during simulator discovery. Derive cached dimensions only after both bin values are positive. | Closed (fixed and verified with UBSan, 2026-09-14) |
| DRV-183 | Medium | `aux_joystick/indigo_aux_joystick.c` (`aux_change_property`, `hotplug_callback`) | Connection work ran on the bus callback and every USB event slept 500 ms before a synchronous full rescan. Connection now reports BUSY and runs on the serialized driver queue; hot-plug callbacks coalesce a delayed queue task and return immediately. | Closed (fixed and verified, 2026-09-14) |
| DRV-184 | High | `aux_joystick/indigo_aux_joystick.c` (`poll`, `open_joystick`, `close_joystick`) | The Linux poller was an unowned `indigo_async` thread; disconnect, reconnect or removal could close/reuse/free its fd and device while it was still running. Open is now transactional and the stored poll thread is atomically stopped and joined before reuse or free. | Closed (fixed and verified with ASan/UBSan, 2026-09-14) |
| DRV-185 | High | `aux_joystick/indigo_aux_joystick.c` (DDHid callbacks and removal) | macOS dispatched events and device destruction concurrently on the global queue and stopped listening asynchronously, permitting property races or callbacks after free. Events now use the serialized driver queue and listener stop completes before synchronous detach/free. | Closed (fixed; production build and shared fake lifecycle verification, 2026-09-14) |
| DRV-186 | High | `aux_joystick/indigo_aux_joystick.c` (`rescan`) | A filename parsed as `jsN` indexed fixed `found[]`/`devices[]` storage without validating N or rejecting suffixes; `js99` could corrupt the stack. Discovery now accepts only exact indices 0–4 and validates all descriptor ioctls. | Closed (fixed and verified with ASan/UBSan, 2026-09-14) |
| DRV-187 | High | `aux_joystick/indigo_aux_joystick.c` (`event_axis`, `event_button`, `event_pov`) | HID event numbers indexed variable-length property item arrays without checking the actual advertised counts. Counts are bounded at attachment and every queued event is validated before property access. | Closed (fixed and verified with ASan/UBSan, 2026-09-14) |
| DRV-188 | Medium | `aux_joystick/indigo_aux_joystick.c` (`DDHidJoystickWrapper.rescan`) | macOS rescan removed wrappers from the same mutable array being fast-enumerated, which can raise a mutation exception. Rescan now matches through a separate mutable copy and removes only after iteration. | Closed (fixed; strict production build, 2026-09-14) |
| DRV-189 | High | `aux_joystick/indigo_aux_joystick.c` (`rescan_handler`) | macOS Bluetooth controllers do not produce libusb hot-plug events, so a disconnected BT pad remained published and Connected indefinitely. A delayed recurring macOS rescan now runs on the serialized driver queue and is removed/drained during shutdown. | Closed (fixed and physically verified with BT unplug/replug, 2026-09-14) |
| DRV-190 | Medium | `ccd_ptp/indigo_ptp_olympus.c` (`ptp_olympus_liveview`) | Queue refactor stopped liveview immediately after a transient GeneralError instead of preserving the original bounded retry policy. A fake-USB/public-bus regression reproduced the failure on 2.0.0.49; original retry policy restored in 2.0.0.50. | Closed (regression and normal/ASan/UBSan suites passed, 2026-09-15) |
| DRV-191 | Medium | `ccd_ptp/indigo_ptp_sony.c` (`ptp_sony_exposure`, `ptp_sony_handle_event`) | NEW_API RAW-first dual capture marked memory readout consumed after RAW, leaving the matching JPEG queued. Physical trace shows DSC08194.ARW at 19:41:57 and its JPEG only during the next capture at 19:43:05. New ordering regression fails on .50; .51 reads the pair across queued finalizers. | Closed (128/128 normal and ASan/UBSan; physical .51 RAW+JPEG and fresh M/L JPEG passed, 2026-09-15) |
| DRV-192 | High | `ccd_sx/indigo_ccd_sx.driver` (`GUIDER_GUIDE_RA` / `GUIDER_GUIDE_DEC` `on_change_request`) | Replacing a running guide pulse deadlocks the driver on real hardware. `on_change_request` runs on the client thread inside `indigo_change_property()`, which holds the bus mutex, and calls `indigo_cancel_pending_handler()`; the still-running `guider_guide_ra_handler` blocks on the same bus mutex in `indigo_update_property()`, so neither side can finish. Reproduced 6 times in 8 runs with SXVR-H694 and SX LodeStar; mutex ownership instrumentation, `sample` and `lldb` stacks recorded below. | Closed (fixed in 3.0.0.16; 24/24 fake USB groups and 8 consecutive hardware guider runs on SX LodeStar, 2026-09-17) |
| DRV-193 | High | `focuser_lunatico/shared/lunatico_shared.c` (`lunatico_command`) | The read loop fills up to `max` bytes and then stores the terminating NUL at `response[max]`, one past the end of the callers' `char response[LUNATICO_CMD_LEN]`; the UDP branch also read a full `LUNATICO_CMD_LEN` regardless of `max`. ASan reports `stack-buffer-overflow` for a 100-byte reply with no `#` terminator. Callers now pass `sizeof(response) - 1` and the UDP read is bounded by `max`. Affects `focuser_lunatico` and `rotator_lunatico`. | Closed (fixed; `oversized` simulator scenario clean under ASan, 2026-09-18) |
| DRV-194 | Medium | `focuser_lunatico/shared/lunatico_shared.c` (`rotator_change_property`, `focuser_change_property`) | A refused goto left the position property BUSY forever. The `FOCUSER_POSITION` goto branch set no failure state at all, and the rotator goto and focuser relative-steps branches set `INDIGO_ALERT_STATE` but then scheduled the 0.5 s poll that immediately overwrote it, so all three assignments were dead. All three now publish ALERT and skip the poll on failure. | Closed (fixed; `goto_failure` scenarios of both Lunatico simulator suites, 2026-09-18) |
| DRV-195 | High | `focuser_lunatico/shared/lunatico_shared.c` (`delete_port_device` / `create_port_device`) | Reconfiguring `LUNATICO_PORT_*_CONFIG` frees a port device through `indigo_detach_device()` but a later `indigo_init_number_property()` reuses the freed property memory. ASan reports `heap-use-after-free` at `indigo_bus.c:1139`. Reproduced by the `exp_rotator`, `third_rotator`, `exp_focuser` and `third_powerbox` scenarios of `test_rotator_lunatico_simulator.c`. | Closed (fixed 2026-09-19 without the deferred rework. Root cause: `configure_ports()` is dispatched with `INDIGO_ASYNC` from three property handlers, so a client changing the model and a port assignment in quick succession runs it on two threads over the same `device_data[].port[]` slots with no synchronisation. Both threads can pass the slot-occupied test and free the same device, or one overwrites the slot while the other attaches, leaving a device registered with the bus that nothing can detach; `indigo_stop()` then reads the freed `device->name`. ASan located the free at `delete_port_device` over memory the allocator had already recycled into a property. The port lifecycle is now serialised by `port_lifecycle_mutex`, taken by `configure_ports`, `INDIGO_DRIVER_INIT` and `INDIGO_DRIVER_SHUTDOWN` (the lock is not taken inside create/delete, which call each other). Verified: an ASan build of `test_rotator_lunatico_simulator` reports no errors and all 11 scenarios pass. Versions: focuser_lunatico `0x0200000A`, rotator_lunatico `0x02000009`.) |
| DRV-196 | Medium | `aux_cloudwatcher/indigo_aux_cloudwatcher.c` (`aag_get_rh_temperature`, `aag_command`) | Low-resolution humidity used the temperature coefficient (`rhi * 1.7572 / 100`) instead of `rhi * 1.25`, reporting -5.3 %RH where the device means 44 %RH, and the precise branch used integer division. Separately, a timeout before the first response byte executed `response[-1] = 0`. Both fixed in driver version `0x0200000A`; previously tracked as `DRV-085`/`DRV-086` in the driver's `REFACTOR.md`. | Closed (fixed; all five simulator scenarios pass, including under ASan, 2026-09-18) |
| DRV-197 | Medium | `focuser_usbv3/indigo_focuser_usbv3.driver` (`usbv3_open`) | `on_disconnect` sends `FQUITx`, which the device answers with `*`, and nothing reads that reply, so a reconnect started with stale lines queued. The identity handshake consumed them instead of `UFO` and the connect failed roughly one run in three. `usbv3_open()` now discards pending input before the handshake. | Closed (fixed; `test_focuser_usbv3_motion` 12/12 after the fix, 2026-09-18) |
| DRV-198 | Medium | 27 drivers, see `REFACTOR.md` note below | `indigo_uni_read_section()` / `indigo_uni_read_line()` treat `length` as the payload capacity and store the terminating NUL at `buffer[length]`, so callers must reserve one extra byte. These drivers passed the full buffer size, so a reply filling the buffer without a terminator wrote one byte past its end: `aux_astromechanics`, `aux_fbc`, `aux_flatmaster`, `aux_flipflat`, `aux_geoptikflat`, `aux_ppb`, `aux_skyalert`, `aux_sqm`, `aux_uch`, `aux_upb`, `aux_upb3`, `aux_usbdp`, `aux_wbplusv3`, `aux_wbprov3`, `aux_wcv4ec`, `dome_skyroof`, `focuser_astromechanics`, `focuser_dmfc`, `focuser_fc3`, `focuser_focusdreampro`, `focuser_mypro2`, `focuser_primaluce`, `focuser_usbv3`, `gps_nmea`, `mount_asi`, `mount_ioptron`, `mount_lx200`, `rotator_falcon`, `wheel_indigo`, `wheel_optec`, `wheel_xagyl`. All now reserve the NUL; every affected driver's version was incremented. | Closed (fixed; full simulator suite green, 2026-09-18) |
| DRV-199 | High | `agent_alpaca/indigo_agent_alpaca.c:378` | The Alpaca PUT handler took `content_length` straight from the client's `Content-Length:` header and used it both as the read length for `indigo_uni_read_line()` and as an index in `buffer[content_length] = 0`, over a `INDIGO_BUFFER_SIZE` heap buffer. A request declaring an oversized length produces an out-of-bounds write at an attacker-chosen offset; the header read on the line above also passed the full buffer size. Both are now bounded. Found while auditing the same call-site class as `DRV-198`; agents were outside the requested sweep, so this fix is offered separately. | Closed (fixed) |
| DRV-200 | High | `mount_lx200/indigo_mount_lx200.c` (`position_timer_callback`) | The position poll rewrote `MOUNT_UTC_ITEM` and `MOUNT_UTC_OFFSET_ITEM` from the mount's own clock on every tick and forced `MOUNT_UTC_TIME` to OK. `MOUNT_UTC_TIME` is handled asynchronously, so a poll landing between `indigo_property_copy_values()` and `mount_set_utc_time_callback()` replaced the client's requested time with the current time: a request for 2026-09-15T23:30:45 offset -3 reached the mount as `:SC09/18/26#`, `:SG+02#`, `:SL10:47:26#`. The forced OK also erased any ALERT from a failed write. The poll now leaves the property alone while it is BUSY, mirroring the existing `MOUNT_HOME` guard in the same function. This was failing `lx200_utc_write_failures_recover` at HEAD, which stopped `test-integration` before the remaining integration tests. | Closed (fixed; 67/67 LX200 simulator scenarios pass) |
| DRV-201 | Medium | `wheel_xagyl/indigo_wheel_xagyl.driver` (`xagyl_open`) | The driver reads model (`I0`), firmware (`I1`) and serial number (`I3`) into `INFO_DEVICE_*` during open, but `INFO_PROPERTY->count` stayed at the base driver's default of 4 and INFO was never re-published, so items 4, 5 and 7 never reached a client: every field read at connect was invisible. `on_attach` now sets the count to 8 and seeds the items, open publishes INFO and close restores "Unknown", matching `aux_flatmaster` and `ao_sx`. | Closed (fixed; `metadata_identity_and_property_completeness` asserts the three values, 2026-09-18) |
| DRV-202 | Medium | `wheel_trutek/indigo_wheel_trutek.driver` (`WHEEL_SLOT` `on_change`) | A move whose reply already carries the destination slot (the device arrived without an intermediate "moving" report) set the item value but published nothing. The block references `wheel_move_finalizer`, so the generator suppresses the final update, leaving `WHEEL_SLOT` BUSY forever. The branch now publishes OK. The bundled simulator always reports one intermediate step, so this path has no simulator coverage; a profile that completes a move in a single exchange would be needed. | Closed (fixed; coverage gap recorded in the driver's `REFACTOR.md`) |
| DRV-203 | Low | `focuser_prodigy/indigo_focuser_prodigy.c`, `focuser_steeldrive2/indigo_focuser_steeldrive2.c` | Both write `INFO_DEVICE_MODEL_ITEM` (index 4) and `INFO_DEVICE_FW_REVISION_ITEM` (index 5) and publish INFO, but neither raises `INFO_PROPERTY->count` above the base driver's default of 4, so the serialized property stops before those items and the values never reach a client. Same class as `DRV-201`. | Closed (fixed 2026-09-19: both drivers now set `INFO_PROPERTY->count = 6` in a device level `on_attach`, seed the two items with "Unknown" and restore them on disconnect, matching DRV-201 and `aux_wbplusv3`. SteelDriveII writes the same identity from its aux device, which got the same treatment. Versions: focuser_prodigy 4 -> 5, focuser_steeldrive2 14 -> 15. Covered: `capabilities` in `test_focuser_prodigy_simulator.c` and `focuser_capabilities` in `test_focuser_steeldrive2_simulator.c` assert the count and both values; removing the count line again fails them with `info->count >= 6`.) |
| DRV-204 | Medium | `agent_guider/indigo_agent_guider.c` | `selection subframe restore` fails deterministically: after guiding with `AGENT_GUIDER_SELECTION.SUBFRAME` set, aborting leaves `CCD_FRAME.WIDTH` at the subframe width (400) instead of restoring the full frame (1600), so the camera stays subframed after the agent stops. Reproduce with `build/integration/test_agent_guider "selection subframe restore"`. Verified pre-existing: it reproduces with `indigo_libs/` reverted to `6c28c23c6`, before this session's changes. Agents were outside the requested sweep, so it is recorded rather than fixed. | Open |
| DRV-205 | Low | `aux_wbplusv3/indigo_aux_wbplusv3.driver`, `aux_wbprov3/indigo_aux_wbprov3.driver` (`AUX_HEATER_OUTLET` and `AUX_POWER_OUTLET_VOLTAGE` `on_change`) | The percentage-to-PWM and volts-to-tenths conversions truncated instead of rounding: `(int)(target * 255.0 / 100.0)` turned a 50 % request into step 127 (49.80 %) rather than 128 (50.20 %), and 25 % into step 63 (24.71 %) rather than 64 (25.10 %). The driver then reported the truncated duty cycle back, so a client saw a value it had not asked for, up to one step (0.39 %) low. The same truncation applied to `20000 + (int)(target * 10)` for the regulated outlet voltage; there the double representation of 13.2 happens to multiply to exactly 132, so no visible error was reproduced, but the expression was wrong for the same reason. All six conversions now use `round()`. Versions: wbplusv3 5 -> 6, wbprov3 5 -> 6. | Closed (fixed; both suites assert the nearest PWM step exactly, and `heater_outlet_round_trips_through_pwm` was confirmed to fail against the truncating driver before the fix was restored, 2026-09-19) |
| DRV-206 | Medium | `aux_wbprov3/indigo_aux_wbprov3.driver` (`on_timer`) | The all-sensors-absent test read `... && PRIVATE_DATA->probe2_temperature && PRIVATE_DATA->probe3_temperature`, testing the last two probes for non-zero instead of for absence (`< -100`). With the DHT22 and probe 1 unplugged and probes 2 and 3 working, `AUX_TEMPERATURE_SENSORS` was published as IDLE although two sensors were reporting; a probe reading exactly 0.00 °C inverted the test the other way. Fixed to compare all four against -100, matching the `aux_wbplusv3` equivalent. Version 4 -> 5. | Closed (fixed; no coverage — the bundled simulator has no option to detach a sensor, so the absent-sensor path cannot be reached from `indigo_test`) |
| DRV-207 | High | `aux_wcv4ec/indigo_aux_wcv4ec.driver` (`on_timer`, `X_COVER_SET_OPEN_CLOSE`) | The poll synced `AUX_SET_OPEN_CLOSE_*_ITEM->number.value` from the device but never `number.target`, while the change handler writes **both** angles from their targets. A client changing only one angle therefore sent the other item's stale target: with the box configured at open 270 / close 20 and the property defined with the compiled-in 110 / 22, changing only the close angle silently reconfigured the open angle from 270 to 110, so the cover would afterwards stop 160 degrees short of open. The 45 degree separation guard reads the same stale targets and could reject a valid pair for the same reason. The poll now assigns `value` and `target` together, and the whole sync is skipped while the property is BUSY. The BUSY guard is part of the fix, not a refinement: syncing the target unconditionally introduced the opposite race, where a poll landing between `indigo_property_copy_values()` and the change handler overwrote the client's fresh target with the device's current angle, so the request was silently replaced by a no-op. That reproduced in 2 of 5 runs before the guard was added and in 0 of 7 after. Version 6 -> 7. | Closed (fixed; `changing_one_angle_leaves_the_other_alone` reproduces the original defect and passes, 2026-09-19) |
| DRV-208 | Low | `aux_wcv4ec/indigo_aux_wcv4ec.driver` (`X_COVER_SET_OPEN_CLOSE` items) | Both angle items are declared `min = 0; max = 295`, but the vendor protocol (`Serial protocol for WandererCover V4-EC(20240702).pdf`, in the driver folder) encodes the open angle as `40000 + x * 100` with `67000` for 270 degrees and the close angle as `10000 + x * 100` with `12055` for 20.55 degrees. Anything above those maxima is sent as a command the device does not implement and is silently ignored, so the request appears to succeed and then reverts at the next status frame. The compiled-in default close angle of 22 degrees is itself above the documented 20.55. Not changed: the PDF gives those two encodings as examples rather than stating a range explicitly, so narrowing the published limits is a judgement call for someone with the hardware. The simulator implements the documented ranges, and the test suite stays inside them. | Closed (fixed 2026-09-19: the open angle is now published as 0..270 and the close angle as 0..20.55, matching the two encodings the vendor PDF documents, and the compiled-in default close angle moved from 22 to 20 so it is inside the range. The conversion also had the DRV-205 truncation, which bites exactly at the new maximum: 20.55 * 100 is 2054.9999999999998 in binary, so `(int)` asked the cover for 20.54; both angles now use `round()`. Version 7 -> 8. Covered: `metadata_and_property_completeness` asserts both ranges and `angles_beyond_the_protocol_range_are_clamped_and_applied` asks for 290/25 and confirms the cover holds 270.00/20.55 across a status frame, which truncation would fail at 20.54.) |
| DRV-209 | Medium | `focuser_lunatico/shared/lunatico_shared.c` (`compensate_focus`) | The fourth and last `lunatico_goto_position()` call site kept the defect DRV-194 fixed at the other three: on a failed goto it set `FOCUSER_STEPS` to ALERT but then unconditionally forced `FOCUSER_POSITION` to BUSY, published it and started the 0.5 s poll. The poll finds the focuser idle at the unchanged position and reports OK, so an automatic temperature compensation that never reached the device is indistinguishable from one that worked. It now publishes ALERT on both properties and returns without polling. Versions: focuser_lunatico `0x0200000A`, rotator_lunatico `0x02000009`. Not covered: the compensation path needs a simulated temperature drift with an injected command failure, which the bundled simulator cannot produce. | Closed (fixed 2026-09-19) |
| DRV-210 | Medium | `focuser_lunatico/shared/lunatico_shared.c` (`handle_rotator_connect_property`) | A rotator sync was silently swallowed on any port whose angle was not already 0. Connect reads the angle from the device into `ROTATOR_POSITION_ITEM->number.value` but never into `PORT_DATA.r_current_position`, which stays 0, and the `ROTATOR_POSITION` handler skips a request whose target equals `r_current_position`: no command, no error, state OK. The framework had already copied the requested 0 into the item, so a client saw the sync succeed while the rotator kept its old angle, and the next goto then travelled the long way round. Connect now assigns both. Versions: focuser_lunatico `0x0200000B`, rotator_lunatico `0x0200000A`. The original record blamed inconsistent degree/step conversions on the non-main ports; that was wrong. The trace behind it was taken before the test's own sequencing was fixed, so the sync it showed was the harmless `lunatico_sync_to_current()` round trip from connect (`getpos 1000` -> 280 deg -> `setpos 1000`) and the test's own sync had been dropped while the property was BUSY. With debug logging the conversions agree in both directions with the defaults (3600 steps/rev, min -180). Covered: `motion_case` in `test_rotator_lunatico_simulator.c` syncs to 0 and then gotos to 20; reverting the fix fails `exp_rotator` and `third_rotator` deterministically, while `main_rotator` passes either way because that port really is at 0. | Closed (fixed 2026-09-19) |
| DRV-211 | Medium | `indigo_tools/indigo_generator` generated `on_timer` prologue, reproduced in `ccd_asi/indigo_ccd_asi.c` | In a generated hot-plug driver the connection handler runs on the per-driver queue and emits `indigo_execute_handler(device, <device>_timer_callback)` immediately before it assigns `CONNECTION_PROPERTY->state = INDIGO_OK_STATE`. The timer callback runs on the device master queue, a different thread, and its generated prologue is `if (!IS_CONNECTED) return;`, which requires that OK state. Waking the device queue usually hands the CPU to it, so the callback frequently observes the still-BUSY connection and returns. Any `on_timer` body that starts a self-rescheduling chain therefore loses the chain for the whole session. Found by inspection, not reproduced. An earlier version of this row claimed a reproduction on a physical ASI294MC Pro, on 2 of 3 connects; that claim is withdrawn. Those failures were caused by the vendor SDK returning `ASI_SUCCESS` with a zero value for the first ~400 ms of `ASI_TEMPERATURE` reads after `ASIInitCamera()` (recorded as ASI-D03 in `ccd_asi/REFACTOR.md`) together with a test that waited for a `CCD_TEMPERATURE` publication the framework legitimately suppresses when nothing changed. A later `--debug` run showed the poll running normally, all three SDK reads every five seconds, while the test still failed, so the window was never observed to fire. It remains a genuine hazard in the generated code, and the old placement had a second problem that needs no thread race at all: the callback could publish an update for a class property before that closing `indigo_<class>_change_property()` call had defined it. 76 `.driver` files use `on_timer`; only generated drivers with a separate driver queue (the `sdk` and `libusb` hot-plug drivers, e.g. `ccd_asi`, `ccd_playerone`, `ccd_svb`, `ccd_atik`, `ccd_qhy2`, `ccd_dsi`, `ccd_iidc`) are exposed, because in a serial driver the timer call is queued behind the connection handler on the same queue. | Closed (fixed in the generator 2026-09-21 with the user's explicit approval. `write_c_connection_change_handler()` no longer emits `indigo_execute_handler(device, <device>_timer_callback)` inside the connect branch; it emits it once at the end of the handler, after `indigo_<type>_change_property(device, NULL, CONNECTION_PROPERTY)` and guarded by `if (IS_CONNECTED)`. That call is what publishes the connection and defines the class properties, so the callback can no longer observe a BUSY connection and can no longer publish to an undefined property; on a failed connect or a disconnect `IS_CONNECTED` is false and nothing is queued. The `ccd_asi` workaround was reverted, so every generated driver now uses the same fixed path. All 115 generator inputs were regenerated; the only change in 67 driver sources is that moved call. No regression test is claimed: the interleaving is not forced by any test in this repository, and the fix removes the window rather than changing an observable contract. Behaviour changes only for the 14 generated hot-plug drivers that queue their connection handler on `driver_queue` and have an `on_timer` block, and those got version bumps: aux_dsusb 13->14, ccd_asi 62->63, ccd_atik 39->40, ccd_atik2 11->12, ccd_dsi 15->16, ccd_fli 11->12, ccd_iidc 17->18, ccd_mi 33->34, ccd_playerone 23->24, ccd_qhy 33->34, ccd_qhy2 33->34, ccd_qsi 15->16, ccd_svb 26->27, focuser_asi 32->33. In a same-queue driver the call was already ordered after the connection handler, so the other 53 drivers are unchanged in behaviour. Verification: all portable and macOS drivers build with no errors, and the narrow fake-SDK suites of 12 of the 14 affected drivers pass, 397 cases in total (ccd_asi 51, ccd_playerone 49, ccd_svb 47, ccd_atik 41, ccd_qhy 37, ccd_qhy2 37, ccd_qsi 60, ccd_mi 17, ccd_iidc 15, ccd_fli 22, focuser_asi 11, aux_dsusb 10); ccd_dsi and ccd_atik2 have no narrow suite. `sensor_only_and_sensor_absent` in `indigo_test/integration/test_ccd_asi_sdk.c` gates the start of the poll, because its `poll_temperature()` needs the callback the poll captures on its first reschedule. The Linux-only `aux_rpio` and `aux_asiair` were regenerated but cannot be compiled on this host.) |
| DRV-212 | Low | 19 generated drivers, audited statically against their pre-migration sources | The migration to `indigo_generator` silently dropped the `multi_device_support` metadata flag wherever the hand-written predecessor had advertised it, because the generator emits `false` unless the `.driver` declares `multi_device_support = true;`. Audited by comparing, for all 115 generator inputs, the current `SET_DRIVER_INFO` multi flag against the last commit of the generated source that had no `.driver` beside it. Result: 13 drivers declare it correctly, 79 never had it, and these 19 lost it: `aux_dsusb`, `ccd_atik`, `ccd_atik2`, `ccd_dsi`, `ccd_playerone`, `ccd_qhy`, `ccd_sx`, `focuser_asi`, `focuser_fcusb`, `guider_asi`, `guider_gpusb`, `polaralign_simulator`, `rotator_asi`, `rotator_falcon`, `rotator_optec`, `rotator_simulator`, `wheel_asi`, `wheel_manual`, `wheel_playerone`. `ccd_qhy2` has no pre-generator history of its own; it was created from `ccd_qhy`, which advertised it. Four of these have a stated intent to preserve it: `ccd_playerone` ("driver metadata advertises multi-device support"), and `wheel_asi`, `focuser_asi` and `rotator_asi` whose goals say "preserve ... multi-device support". `wheel_playerone/REFACTOR.md` instead documents `false` as a "convention as `wheel_asi`", which codified the accident rather than deciding it. The flag has no consumer inside this repository — only the generator, the tests and two library declarations reference it — so the impact is on external clients reading `indigo_driver_info`, which is why this is Low rather than a functional defect. | Closed (all 20 fixed 2026-09-21). `ccd_asi` (ASI-D01, 61 -> 62) and `ccd_playerone` (POA-D01, 24 -> 25) were fixed with their hardware passes; the remaining 19 followed in one sweep: `aux_dsusb` 14 -> 15, `ccd_atik` 40 -> 41, `ccd_atik2` 12 -> 13, `ccd_dsi` 16 -> 17, `ccd_qhy` 34 -> 35, `ccd_qhy2` 34 -> 35, `ccd_sx` 19 -> 20, `focuser_asi` 33 -> 34, `focuser_fcusb` 8 -> 9, `guider_asi` 9 -> 10, `guider_gpusb` 8 -> 9, `polaralign_simulator` 3 -> 4, `rotator_asi` 6 -> 7, `rotator_falcon` 8 -> 9, `rotator_optec` 3 -> 4, `rotator_simulator` 6 -> 7, `wheel_asi` 15 -> 16, `wheel_manual` 5 -> 6, `wheel_playerone` 11 -> 12. Every regenerated source differs only by the version and the `SET_DRIVER_INFO` flag. Twelve narrow tests declared the old value in their `simulator_driver_case` and were updated, which is what asserts the flag for those drivers. Verification: all portable and macOS drivers build with no errors, and the narrow suites of the 18 swept drivers that have one pass, 306 cases in total (`ccd_atik` 41, `ccd_qhy` 37, `ccd_qhy2` 37, `ccd_atik2` 24, `ccd_sx` 24, `focuser_fcusb` 17, `guider_asi` 16, `guider_gpusb` 15, `rotator_falcon` 13, `wheel_playerone` 12, `focuser_asi` 11, `rotator_asi` 11, `rotator_optec` 11, `polaralign_simulator` 11, `aux_dsusb` 10, `rotator_simulator` 9, `wheel_asi` 5, `wheel_manual` 2). `ccd_dsi` has no narrow suite, so its bump is build-verified only. The misleading note in `wheel_playerone/REFACTOR.md` that had recorded `false` as a deliberate convention was corrected. |
| DRV-213 | Medium | `indigo_drivers/ccd_touptek/indigo_ccd_touptek.c` and the framework `config_restore` machinery in `indigo_libs/indigo_driver.c` | `CONFIG LOAD` fails for the whole ToupTek OEM driver family. Reproduced on a physical Touptek GPCMOS01200KMB and an Altair ALTAIRGP224C: `CONFIG SAVE` writes a correct file, verified by reading it back, but the load answers "Configuration restore failed or timed out" and applies nothing, with no vendor SDK call at all between the request and the failure. Ruled out by experiment: the save itself, a saved property left un-settled (all fourteen are published and OK when the failure is reported), `CCD_LENS` settling in `INDIGO_IDLE_STATE` while zeroed (configuring it made no difference), and the framework suppressing an update whose values already match the saved ones (moving a saved value away first made no difference). The remaining suspect is the restore observer's completion detection against this driver's hand-written `ccd_change_property()`, which returns `INDIGO_OK` without publishing anything when a request arrives while a related property is BUSY — the silent-drop pattern the generated drivers replaced with an explicit `reject_change` ALERT, and which a waiting client cannot distinguish from a lost request. Confirming it means changing `indigo_libs`, which was outside the scope of this driver pass. Full evidence and the ruled-out hypotheses are in `indigo_drivers/ccd_touptek/REFACTOR.md` (TT-D03). | Closed (fixed 2026-09-21, hardware re-test still owed). The silent drop was confirmed under the fake SDK: holding `CCD_EXPOSURE` BUSY across a `CONFIG LOAD` makes the restore wait on `X_CCD_BIN_MODE` with no update at all and lose `CCD_GAIN`, which is last but one in the file. Two changes. `ccd_touptek` answers the six cross-property interlocks with an explicit `reject_change()` ALERT instead of a bare `INDIGO_OK`, leaving the self-BUSY case to `INDIGO_COPY_*_PROCESS_CHANGE` so a running operation's state is never overwritten; version `0x03000030` -> `0x03000031`. `config_restore` in `indigo_libs/indigo_driver.c` now records and skips a property that answers ALERT or publishes nothing within `CONFIG_RESTORE_ACK_TIMEOUT`, applies the rest of the file, and names the refused properties in the `CONFIG` ALERT message (LIB-015). Regression test `Configuration restore survives a refused property` in `indigo_test/integration/test_ccd_touptek_sdk.c` fails against the pre-fix framework at the `CCD_GAIN` assertion and passes after it; the suite is 30/30. The hardware case `tp_saves_and_loads_configuration` is unchanged and was not re-run, because the cameras were not available; the earlier claim that the update-suppression and `CCD_LENS` hypotheses were the remaining suspects is superseded, and which property the two cameras actually refuse is still unknown. The fixed restore reports it by name.) |
| DRV-214 | Low | `ccd_asi/indigo_ccd_asi.c:1526,1534`, `ccd_mi/indigo_ccd_mi.c:703`, `ccd_playerone/indigo_ccd_playerone.c:1716,1724`, `ccd_sbig/indigo_ccd_sbig.c:1402`, `ccd_simulator/indigo_ccd_simulator.c:1288,1296,1483,1491`, `ccd_svb/indigo_ccd_svb.c:1278,1286` | The same silent-drop shape DRV-213 fixed in the ToupTek camera device survives in eleven more `change_property` branches: a guard returns `INDIGO_OK` without publishing anything, so a client that waits for a response cannot tell the refusal from a lost request. Found by a static sweep of every `change_property` body under `indigo_drivers/`, `indigo_linux_drivers/` and `indigo_mac_drivers/` for `if (...) { return INDIGO_OK; }` blocks containing no `indigo_update_property()`, `INDIGO_UPDATE_PROPERTY_STATE()`, `reject_*()` helper or `INDIGO_COPY_*_PROCESS_CHANGE`. `mount_mxhd` and `ccd_ssag` matched the text but answer through `reject_if_parked()` / `ensure_motor_recovery()` / `INDIGO_UPDATE_PROPERTY_STATE()` and are correct. All but the `ccd_sbig` and `ccd_simulator` hits sit inside generator `on_change_request` blocks, so the fixes belong in the `.driver` inputs and need regeneration plus version bumps. Ten of the eleven guard an acquisition (`CCD_EXPOSURE`/`CCD_STREAMING` BUSY); `ccd_mi:703` guards `CCD_TEMPERATURE_PROPERTY->perm == INDIGO_RO_PERM`, which `indigo_property_match_changeable()` already excludes, so that one is dead code rather than a live drop. The ToupTek focuser had the same shape and was fixed with DRV-213. Severity is Low because the LIB-015 change stops any of these from breaking a configuration restore; what remains is a client that gets no answer to a refused request. | Closed (fixed 2026-09-21). The eleven were resolved as ten refusals plus one piece of dead code. `ccd_asi` (64 -> 65), `ccd_svb` (27 -> 28), `ccd_playerone` (25 -> 26) and `ccd_simulator` (27 -> 28, ten blocks across its five camera devices) now use the generator's `reject_change` for the cross-property interlock, which emits the house pattern: every item marked `do_update`, `INDIGO_ALERT_STATE` and a message. `ccd_playerone` guarded `CCD_EXPOSURE || CCD_STREAMING` in both branches; only the other property is rejected now, because publishing ALERT onto the property that is running the operation would overwrite the state its own interlock reads. That self-BUSY case stays with `INDIGO_COPY_*_PROCESS_CHANGE` and is covered by the LIB-015 acknowledgement timeout. `ccd_mi`'s guard was unreachable dead code (`perm == INDIGO_RO_PERM`, already excluded by `indigo_property_match_changeable()`) and was removed without a version bump, since nothing changes behaviourally. `ccd_sbig:1402` was reclassified and deliberately left alone: it guards `CCD_EXPOSURE` against its own BUSY state, a hand-written equivalent of the macro guard, so rejecting it would corrupt the running exposure. Regression coverage: an assertion was added to an existing case in each of `test_ccd_asi_sdk` (`Edges rejected_change_alerts_and_keeps_values`), `test_ccd_svb_sdk` (same), `test_ccd_playerone_sdk` (`property_busy_guard`) and `test_ccd_simulator` (`simulator_stream_abort_and_reconnect`); each requests streaming during an exposure and requires `CCD_STREAMING` to reach ALERT, and each fails at exactly that assertion against its pre-fix driver. Suites after the fix: ccd_asi 51/51, ccd_playerone 49/49, ccd_svb 47/47, ccd_simulator 19/19, ccd_mi 17/17, and `make all` builds clean. No new test cases were registered, so the `Automated Tests (Sim / HW)` counts are unchanged. The refusal itself then moved into the bus as `indigo_reject_change()` behind `INDIGO_REJECT_CHANGE_IF()` (LIB-016), which the generator now emits in place of the inline eight-line block, so these four drivers and the other 45 with a `reject_change` share one implementation.) |
| DRV-215 | Medium | `ccd_touptek/indigo_ccd_touptek.c` `ccd_event_handler()` | The pull-mode event handler cancelled the exposure watchdog for every SDK notification before checking the event type, and did not handle `EVENT_TRIGGERFAIL`. An unrelated notification or a trigger failure therefore left a single exposure BUSY with no watchdog. Also, with `OPTION_NOPACKET_TIMEOUT` disabled (the SDK default), a frame lost on USB produced no notification at all, so a guide exposure waited exposure + 25 s for the watchdog. Seen with a GPM462M guider next to a SkyEye 26AM plus imager. | Closed (fixed 2026-09-25, hardware confirmation pending). The watchdog is now cancelled only by the image and failure events, `EVENT_TRIGGERFAIL` is handled as a failure, and the no-packet timeout is armed (exposure * 102% + 4 s) only while an acquisition runs. Version `0x03000031` -> `0x03000032`. Regression test `Acquisition no-packet timeout and unrelated SDK events` in `indigo_test/integration/test_ccd_touptek_sdk.c`. Details are in `ccd_touptek/REFACTOR.md` (TT-D04). |
| DRV-216 | Medium | `ccd_touptek/indigo_ccd_touptek.c` camera, wheel and focuser `change_property` and connection handlers | `CONFIG` requests were queued through `INDIGO_COPY_VALUES_PROCESS_CHANGE`. A connection change cancels pending handlers and then set `CONFIG` from BUSY to ALERT in memory only, without publishing it. `CONFIG` is never deleted or redefined, so clients kept showing it BUSY. While `CONFIG` was BUSY, new requests were also dropped silently. | Closed (fixed 2026-09-25, hardware not verified). `CONFIG` is handled on the bus thread as in the generated drivers; the config handlers and the in-memory `CONFIG` reset are removed. Version `0x03000032` -> `0x03000033`. Regression test `Configuration request survives a connection change` in `indigo_test/integration/test_ccd_touptek_sdk.c`, which also now isolates its configuration through a private HOME. Details are in `ccd_touptek/REFACTOR.md` (TT-D05). |
| DRV-217 | Medium | `indigo_tools/indigo_generator.c:1676` and every generated driver | The generated disconnect branch calls `indigo_cancel_pending_handlers(device)` but does not reset properties whose queued change handler it cancelled. Such a property stays BUSY in memory. A connected-only property is redefined BUSY on the next connect, and an `always_defined` one stays BUSY for clients straight away. Either way the `INDIGO_COPY_*_PROCESS_CHANGE` BUSY guard then silently drops every later change until the driver restarts. Reproduced on 2026-09-25 with the `ccd_asi` fake SDK (temporary probe, not committed): the camera queue was held, `CCD_GAIN` was changed and the camera disconnected; after reconnect `CCD_GAIN` was defined BUSY and a new gain request was ignored. The risk is highest in the hot-plug drivers, whose connection handler runs on the driver queue in parallel with the device queue (`ccd_asi`, `ccd_playerone`, `ccd_svb`, `ccd_fli`, `ccd_mi`, `ccd_sx`, `ccd_atik`, the ASI, PlayerOne, FLI, SX and MI wheels and focusers, and others). The 29 `always_defined` properties with a queued `on_change` (outlet and sensor names in `aux_*`, `dome_dragonfly`, `focuser_lunatico`, `rotator_lunatico`, `focuser_prodigy`; `mount_pmc8` `CONNECTION_MODE`/`MOUNT_TYPE`; `X_AUX_PWM` in `aux_asiair`/`aux_rpio`; `X_ANEMOMETER_TYPE` in `aux_cloudwatcher`) are all in serial drivers that connect on the device queue, so only a request arriving between a DISCONNECT and its handler is exposed. Hand-written drivers that cancel pending handlers were not audited individually; `ccd_touptek` resets these properties itself (see DRV-216). The fix needs a generator change and therefore user approval. | Closed (fixed 2026-09-25 in the generator, TOOLS-014, and in `ccd_touptek`). As agreed with the user, a new session starts in a clean state: generated drivers return every declared property still BUSY after `on_disconnect` to OK, and republish `always_defined` ones. The 114 generated drivers with declared properties were regenerated with a version bump. `ccd_touptek` resets these properties to OK instead of ALERT and publishes `X_WHEEL_MODEL`, which stays defined while disconnected (TT-D06, version `0x03000034`). Regression tests: `Cancelled change does not survive disconnect` (`test_ccd_asi_sdk`) and `Cancelled change does not survive a connection change` (`test_ccd_touptek_sdk`); both fail against the previous code. Hand-written drivers other than `ccd_touptek` that cancel pending handlers were not audited. |
...
## Configuration Agent — initial behavioral test audit (2026-09-10)

User-requested scope: `agent_config` and a new hardware-free test suite. Production source was inspected in full for test planning, together with `git diff 017ba602857378e4aed489c065c76eacae15924c..HEAD -- indigo_drivers/agent_config`; the observed HEAD was `a321d776c459f9a07295d2eca611daac5b7ffcd3`. This is a scoped audit, not a completed review of all drivers; the folder baseline is unchanged. At this initial audit, production agent code and version `0x03000007` were unchanged; subsequent fixes are recorded below. Standard AGENT_CONFIG names were checked against `indigo_names.h` and the documented Configuration agent section; no property was added or removed.
...
Findings DRV-148–DRV-168 are recorded in the main findings table above. Every finding was reproduced with a failing behavioral regression; those tests now pass after the individual fixes below.

### Validation and limits

- Universal macOS arm64/x86_64 compilation succeeded. Execution on arm64: **30/55 cases passed, 25 failed**, corresponding to **21 distinct findings** above. These were the initial, pre-fix results. The fixes and final successful validation are recorded below. Run an individual case by passing its name to `indigo_test/build/integration/test_agent_config`.
...
### DRV-162 fix validation — 2026-09-10

Serialization failures now propagate from the framework. The agent writes a temporary file, checks serialization and synchronization, and uses a portable replace helper only on success. Tests verify the original snapshot survives injected serialization, actual descriptor write errors and replacement failure, temporary cleanup, successful overwrite and reload. Shared I/O implementation uses fsync/rename on POSIX and _commit/MoveFileExA on Windows; Windows execution remains unverified. Tests passed: `save_write_failure`, `overwrite`, `save_roundtrip`, `nondefault_port_load`, `save_empty_and_failure`. Universal macOS build; hardware-free arm64 execution.

Shared-library validation: `make -C indigo_libs` and `make -C indigo_test test-unit` passed (150 unit cases).

### DRV-163 fix validation — 2026-09-10

An empty LOAD selection is now acknowledged as a no-op before scheduling any worker or changing LAST, device selections or related agents. Tests passed: `empty_load`, `busy_guard`, `save_roundtrip`. Universal macOS build; hardware-free arm64 execution.

### DRV-164 fix validation — 2026-09-10

SAVE and REMOVE now share basename validation, rejecting directory traversal, absolute paths, Windows separators/drive syntax and dot-directory names before any file operation. Valid names containing dots still work. Tests passed: `save_path_separator`, `save_empty_and_failure`, `remove_roundtrip`, `save_roundtrip`. Universal macOS build; hardware-free arm64 execution.

### DRV-165 fix validation — 2026-09-10

Filter definition, update and deletion now consistently require the complete FILTER_ prefix. Tests passed: `filter_prefix_consistency`, `all_filter_classes`, `deletion`, `remote_ignored`. Universal macOS build; hardware-free arm64 execution.

### DRV-166 fix validation — 2026-09-10

SETUP now propagates persistence/flush failure to its visible result instead of only changing hidden CONFIG. The test confirms successful persistence after a failed attempt. Tests passed: `setup_save_failure`, `setup_persistence`, `setup_restart`, `autosave`. Universal macOS build; hardware-free arm64 execution.

### DRV-167 fix validation — 2026-09-10

Agent snapshots retain each filter status and derive their aggregate status without allowing later successful updates to hide ALERT or BUSY. Status arrays follow insertion, compaction, removal and detach. Tests cover masked errors, masked BUSY timeouts, recovery and deletion of the failing filter. Tests passed: `selection_alert_masked`, `selection_failure`, `related_failure`, `restore_busy_timeout`, `busy_then_ok`, `deletion`, `agent_capacity_reuse`, `save_roundtrip`. Universal macOS build; hardware-free arm64 execution.

### DRV-168 fix validation — 2026-09-10

Driver restoration now merges saved selections with the complete current server inventory. Unload-unused explicitly disables newly introduced drivers; the default policy retains them. Rejection, repeated restoration and ordinary roundtrip remain covered. Tests passed: `driver_unload_new`, `driver_unload_policy`, `driver_rejection`, `save_roundtrip`, `repeated_load`. Universal macOS build; hardware-free arm64 execution.

### Configuration Agent — final fix validation (2026-09-10)

All findings DRV-148–DRV-168 are closed, with one tested commit per finding. Each fix increments the driver version; the final version is `0x0300001C` (initial `0x03000007`). Final production library and agent builds succeeded for macOS arm64/x86_64. All **56/56 integration cases pass**, including cleanup; the full AddressSanitizer build also passes **56/56**, with no reported memory errors in the instrumented agent/framework/harness. Shared-library changes passed **150/150 unit cases**. The original-duration restore timeout, missing-profile recovery and save/load roundtrip tests also pass. The initial coverage percentages above refer to the pre-fix source and were not remeasured for this final revision.

DRV-162 includes portable shared I/O file synchronization/replacement and serialization error propagation; all other behavior changes are in the Configuration Agent. No property was added or removed. Windows/Linux execution, real driver loading and exhaustive scheduling interleavings remain unverified. User changes in the Xcode project and driver-editor/template files were not staged. No push was performed. Test build artifacts were removed after validation.

### Scripting Agent — fix validation (2026-09-14)

Findings DRV-169–DRV-178 are closed in the working tree. The new real-Duktape/public-bus integration suite passes **35/35** cases in the macOS arm64 runtime, including script/configuration lifecycle, all native bindings, `boot.js`, 128-script and cached-property capacities, timer and callback failures, shutdown and recovery. `library/Sequencer.js` is excluded by user request. The first AddressSanitizer run reproduced DRV-178 as a heap buffer overflow at the 127th script; the corrected arm64 ASan/UBSan build passes **35/35** with no reported sanitizer error. macOS leak detection is unavailable.

The production agent and release test executable build for macOS arm64/x86_64. Final LLVM coverage of `indigo_agent_scripting.c` is 70/70 functions (100%), 1,376/1,443 lines (95.36%), 1,853/1,999 regions (92.70%) and 469/588 branches (79.76%). Linux/Windows execution and physical hardware are not applicable/available. The agent version increases from `0x0300000B` to `0x0300000C`; no property was added or removed.

### Sequencer.js and Guider follow-up — fix validation (2026-09-14)

Findings DRV-130 and DRV-179–DRV-182 are closed in the working tree. DRV-130 uses the approved cosine/sine unit-vector projection and retains its focused magnitude regression. The separate real-Duktape Sequencer corpus inventories and invokes all **106/106** public methods, validates every generated microstep, and passes **25/25** hardware-free cases using the real Imager, Mount and Guider agents with CCD, wheel, focuser, mount, dome, GPS and rotator simulators. Configuration, server loading and astrometry use deterministic public-bus peers. The original Scripting Agent corpus still passes **35/35**.

The arm64 ASan/UBSan Sequencer build passes **25/25** without AddressSanitizer errors. UBSan exposed DRV-182 in the Guider Agent; the focused sanitizer regression passes after the fix. Vendored Duktape still reports its intentional `-1` to unsigned sentinel conversion in `duk_hobject_props.c`. The complete Guider corpus passes 88/88 cases, including `dither RA projection preserves magnitude`. Strict syntax checks and universal production builds pass for both affected agents. Scripting Agent is now `0x0300000D`; Guider Agent is `0x0300002F` for DRV-182.

### AUX Joystick queue and fake-HID validation — 2026-09-14

Findings DRV-183–DRV-189 are closed in the working tree. The new driver-specific fake Linux HID suite compiles the production Linux backend against deterministic filesystem, joystick-event and libusb boundaries while retaining the real bus, serialized driver queue and pthread poller. All 14/14 cases pass normally and with the driver plus harness instrumented by ASan/UBSan. The cases cover metadata and properties, reported-count bounds, queued BUSY connection, the connection BUSY guard, open/fcntl failure and recovery, axes/buttons/mapping/options, invalid events, disconnect/reconnect quiescence, non-blocking coalesced hot-plug, invalid discovery and descriptor recovery, five-device attach/survivor behavior, connected unplug, rejected shutdown, queue draining and reinitialization.

Strict macOS Objective-C driver compilation and strict fake-Linux/harness compilation pass with warnings treated as errors (excluding unused helpers from the shared test header). The normal production archive, dylib and executable build for macOS arm64/x86_64; existing warnings are confined to bundled DDHidLib. Measured fake-Linux/common-source coverage is 20/21 functions (95.24%), 494/685 lines (72.12%), 1,129/1,611 regions (70.08%) and 199/401 branches (49.63%). A physical Bluetooth pad passed discovery, queued connect/disconnect/reconnect, D-pad axis events, button press/release events and connected unplug/replug; the first unplug exposed DRV-189 and the repeated cycle passed after the fix. A Logitech Dual Action USB controller then passed hot-plug discovery, queued connection, smooth full-range `-65536…65536` input on four analog axes, discrete D-pad/POV input on two axes, button press/release events, connected unplug, replug and reconnect. A Linux host and Windows remain untested. Driver version changes from `0x02000008` to `0x0300000A`; no property was added or removed. Folder review baseline remains unchanged.


### Scoped Olympus original-driver comparison — 2026-09-15

Compared the complete Olympus/OM System working-tree refactor against original HEAD `85f73a49fa177e126c4d326b7cf99892f7e7bdf1`, including shared acquisition/event/stream lifecycle changes. DRV-190 reproduced immediate stream failure after one GeneralError and restores the original bounded error retries in version 2.0.0.50. Initialization, vendor conversion and focus command encoding are unchanged. Native vendor-event coverage now exercises C103, C102, legacy second-card duplication and C108 through the real driver interrupt transport.

Remaining unverified risk: original raw USB polling/reset loops now share the queue with shutter release and abort, so long replies/recovery can delay them. Physical Olympus BULB timing, model-specific descriptors/RAW+JPEG and reset/dial-change sequences remain unverified without a camera. See `ccd_ptp/REFACTOR.md` for test mapping and results. This is a scoped comparison; the folder-wide last-reviewed commit remains unchanged.

Final scoped verification: 125/125 hardware-free cases passed normally and under ASan/UBSan (16 Olympus cases in the fake-USB suite). Universal macOS production and strict Olympus compilation passed. No Olympus hardware tests ran. The ICA event fixture stack overflow was corrected by heap allocation; no production ICA change.

### SX physical camera acceptance and DRV-192 — 2026-09-17

Ran the new opt-in `indigo_test/hardware/test_ccd_sx_hw.c` against a Starlight Xpress SXVR-H694 (2750 x 2200, cooled, Star2K) and an SX LodeStar (752 x 580, interlaced, uncooled, Star2K) on macOS 26.6.2 arm64 with the unmodified driver `0x0300000F`. Identity and property contract, shared CCD/guider lifecycle with rejected shutdown, acquisition (frame types, 1x1/2x2/4x4 bins, ROI, clamped frames, rejected bins, long-exposure register clear), abort orders, cooling, guider pulse timing, flood-LED visibility, INIT/SHUTDOWN reload and physical unplug/replug while idle and during an exposure all passed. Detailed evidence is in `indigo_drivers/ccd_sx/REFACTOR.md`.

DRV-192 was found by the guider case. The client thread sends a new pulse on an axis whose previous pulse is still BUSY, which is the same replacement scenario the fake-USB suite covers; with real USB timing the previous handler is frequently still running when the replacement arrives. Captured state at the deadlock:

- client thread: `indigo_change_property` (bus mutex held) -> `guider_change_property` -> `indigo_cancel_pending_handler` -> `indigo_queue_remove` waiting for the running task;
- device queue thread: `guider_guide_ra_handler` -> `indigo_update_property` blocked on the same bus mutex;
- instrumented ownership dump: the single `bus_mutex` (aliased as both `client_mutex` and `device_mutex` in `indigo_libs/indigo_bus.c`) is held by the client thread from `indigo_bus.c:672`, with the device queue thread waiting for it at `indigo_bus.c:887`.

The hazard is not limited to SX: any driver that calls `indigo_cancel_pending_handler()` from `on_change_request` or directly from `change_property` can block the bus thread while the cancelled handler needs the bus mutex to publish an update. The fix belongs to the driver (or generator) and possibly to the framework lock scope; it was not attempted here because hardware validation was the requested scope. The hardware harness now enforces a per-case deadline so the deadlock fails the case instead of hanging the run.

### DRV-192 fix validation — 2026-09-17

Driver version 3.0.0.16 keeps the requested replacement semantics and removes the deadlock: `on_change_request` no longer cancels the queued change handler from the client thread, and the replacement is done by cancelling the pending finalizer inside the change handler, which already runs on the device queue that the finalizer belongs to, so `indigo_queue_remove()` returns without waiting. The guider `on_disconnect` also clears the items and returns both properties to `INDIGO_OK_STATE` so an interrupted pulse cannot leave the property BUSY.

The one behavioural consequence is that requests piling up behind an occupied device queue are no longer discarded by cancelling the queued handler; each performs its own replacement, so three requests arriving during a readout now produce four relay commands instead of two, with the last request still winning. `test_ccd_sx_usb.c` asserts the new accounting.

Validation: all 24 fake USB groups pass, and on hardware the guider case passed 8 consecutive runs on an SX LodeStar where 3.0.0.15 deadlocked in 6 of 8. Measured replacement: an 800 ms pulse replaced after 200 ms by a 200 ms pulse ends after 0.408–0.421 s, a zero request cancels a running 3000 ms pulse after 0.313–0.318 s. The whole opt-in hardware suite passes with the LodeStar; the SXVR-H694 was unplugged during this validation and was not retested with 3.0.0.16.
