...
| DRV-124 | High | `agent_guider/indigo_agent_guider.c:728` | RAW header, dimensions and pixel payload are now validated before image access, with division checks avoiding overflow and signed-int limits matching image/Bayer helpers. Version increment deferred until DRV-133 is fixed, as part of one combined change; all four RAW integration cases pass normally and under AddressSanitizer, including short headers, invalid dimensions, all four pixel formats, Bayer metadata and recovery. | Closed (fixed) |
