...
| DRV-130 | Medium | `agent_guider/indigo_agent_guider.c:2684` | RA-only dithering uses `dith_total / (cos_angle + tan_angle)` rather than a unit-vector projection, so the requested magnitude is not preserved. `integration/test_agent_guider.c` reproduces this in `dither RA projection preserves magnitude`. The proposed cosine/sine projection was declined for now and is retained only as a commented proposal in the source. Any implementation change requires manual approval; do not fix DRV-130 automatically when running or maintaining the Guider Agent tests. | Open (awaiting manual approval, 2026-09-14) |
...
Findings DRV-179–DRV-182 are closed in the working tree. The proposed fix for DRV-130 was declined and the finding remains open pending manual approval. Its regression must remain visible and must not trigger an automatic code fix during future Guider Agent testing. The separate real-Duktape Sequencer corpus inventories and invokes all **106/106** public methods, validates every generated microstep, and passes **25/25** hardware-free cases using the real Imager, Mount and Guider agents with CCD, wheel, focuser, mount, dome, GPS and rotator simulators. Configuration, server loading and astrometry use deterministic public-bus peers. The original Scripting Agent corpus still passes **35/35**.

The arm64 ASan/UBSan Sequencer build passes **25/25** without AddressSanitizer errors. UBSan exposed DRV-182 in the Guider Agent; the focused sanitizer regression passes after the fix. Vendored Duktape still reports its intentional `-1` to unsigned sentinel conversion in `duk_hobject_props.c`. The complete Guider corpus passes 87/88 cases; `dither RA projection preserves magnitude` continues to reproduce the open DRV-130 finding. The Sequencer simulator-guiding scenario passes because it does not assert RA-only projection magnitude. Strict syntax checks and universal production builds pass for both affected agents. Scripting Agent is now `0x0300000D`; Guider Agent is `0x0300002F` for DRV-182.
