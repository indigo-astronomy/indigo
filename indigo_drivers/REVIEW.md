...
| DRV-203 | Low | `focuser_prodigy/indigo_focuser_prodigy.c`, `focuser_steeldrive2/indigo_focuser_steeldrive2.c` | Both write `INFO_DEVICE_MODEL_ITEM` (index 4) and `INFO_DEVICE_FW_REVISION_ITEM` (index 5) and publish INFO, but neither raises `INFO_PROPERTY->count` above the base driver's default of 4, so the serialized property stops before those items and the values never reach a client. Same class as `DRV-201`. | Open (not covered by either driver's current tests) |
