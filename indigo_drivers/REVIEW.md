...
| DRV-124 | High | `agent_guider/indigo_agent_guider.c:724` | Malformed RAW BLOBs are dereferenced without checking the header or pixel payload length. `truncated raw` delivers a four-byte RAW signature and ASan reports a heap-buffer-overflow in `capture_frame()` while reading header dimensions. Validate header size, dimensions and overflow-safe payload length before image access. Reproduced by `integration/test_agent_guider.c`: truncated raw. | Open |
| DRV-125 | High | `agent_guider/indigo_agent_guider.c:3128` | SHUTDOWN detaches the filter client and frees cached properties before stopping the active process. ASan in `shutdown active` reports a heap-use-after-free in `indigo_restore_switch_state()` called by the guiding worker; the freeing stack is `indigo_filter_client_detach()`. Stop and join active work before releasing the client/cache. Reproduced by `integration/test_agent_guider.c`: shutdown active. | Open |
| DRV-126 | Medium | `agent_guider/indigo_agent_guider.c:1240` | Single preview ignores the result of `capture_frame()`, marks a successful image FAILED in phase statistics, and assigns a boolean comparison to the process state (normally IDLE). Both successful capture and three failed exposure attempts publish IDLE instead of the required OK/ALERT. Preserve the capture result and abort reason when publishing completion. Reproduced by `integration/test_agent_guider.c`: single preview status and format restoration; single preview failure. | Open |
| DRV-127 | Medium | `agent_guider/indigo_agent_guider.c:1267` | Continuous preview clears abort BUSY before testing it for the final phase, so an explicit abort is reported as FAILED. Its boolean assignment to the process state publishes IDLE both on abort and on camera failure/disconnection. Publish DONE/OK for abort and FAILED/ALERT for acquisition failure using the original termination reason. Reproduced by `integration/test_agent_guider.c`: continuous preview abort status; camera disconnect. | Open |
| DRV-128 | Medium | `agent_guider/indigo_agent_guider.c:1292` | Adaptive calibration cannot reduce a too-large step: `change_step()` compares the current step with its maximum instead of minimum before halving. With 10 px/s motion, a 0.2 s step and 3 px calibration threshold, calibration immediately fails instead of reducing the step and retrying. Reproduced by `integration/test_agent_guider.c`: calibration adaptive step. | Open |
| DRV-129 | Medium | `agent_guider/indigo_agent_guider.c:1470` | Calibration computes speed using zero-based loop index `i` after issuing `i + 1` pulses; the west and east paths use the same undercount at lines 1552 and 1586. A deterministic 10 px/s camera/guider model is calibrated as 12.5 px/s. Divide measured drift by the actual accumulated pulse duration. Reproduced by `integration/test_agent_guider.c`: calibration speed accuracy. | Open |
...
| DRV-131 | Medium | `agent_guider/indigo_agent_guider.c:2035` | The inter-frame guiding delay loop never checks abort. A four-second delay prevents abort completion for most of those four seconds; the bounded 1.5-second responsiveness regression fails. Check abort within the existing delay loop and finalize promptly. Reproduced by `integration/test_agent_guider.c`: guiding delay abort. | Open |
| DRV-132 | Medium | `agent_guider/indigo_agent_guider.c:1225` | `pulse_guide()` returns OK unconditionally after waiting for pulse completion, ignoring RA/Dec ALERT and a BUSY timeout. An injected guider ALERT leaves guiding running and issuing further commands. Propagate pulse failure so calibration/guiding can terminate or recover rather than pretending the correction succeeded. Reproduced by `integration/test_agent_guider.c`: pulse error. | Open |
| DRV-133 | Medium | `agent_guider/indigo_agent_guider.c:1788` | An exact zero drift skips the entire correction/statistics/history block. After a measured two-pixel displacement returns exactly to the reference, published DRIFT_RA remains 2 and the previous correction is retained; zero-error samples also never reach correction histories or the PPEC model. Update statistics and model history for every accepted frame, even when no pulse is required. Reproduced by `integration/test_agent_guider.c`: zero drift statistics. | Open |
