...
| DRV-210 | Medium | `focuser_lunatico/shared/lunatico_shared.c` (rotator `ROTATOR_POSITION` sync path) | On the exp and third ports a sync to 0 deg leaves the driver publishing roughly 280 deg. Command trace: the port reads `!step getpos 1 -> 1000`, the sync writes `!step setpos 1 1000` and the following goto to 20 deg writes `!step goto 1 2000`, so the change handler is converting with 50 steps/deg and a 1000 step offset, while the value published from the poll moves 10 steps per degree with a 180 deg offset (1021 steps published as 282.1 deg). The two conversions agree again at the goto target, which is why the move still ends on the requested angle. The main port does not show it. Either `ROTATOR_STEPS_PER_REVOLUTION`/`ROTATOR_LIMITS_MIN_POSITION` are not the same on both paths for a port device, or the poll publishes a conversion made for another port. Found while fixing DRV-195, which used to crash these scenarios before they got this far. Not fixed: the cause is not pinned down and the arithmetic is shared with the focuser side. `motion_case` in `test_rotator_lunatico_simulator.c` documents it and asserts the goto end to end without asserting the post-sync angle. | Open |
