...
| DRV-195 | High | `focuser_lunatico/shared/lunatico_shared.c` (`delete_port_device` / `create_port_device`) | Reconfiguring `LUNATICO_PORT_*_CONFIG` frees a port device through `indigo_detach_device()` but a later `indigo_init_number_property()` reuses the freed property memory. ASan reports `heap-use-after-free` at `indigo_bus.c:1139`. Reproduced by the `exp_rotator`, `third_rotator`, `exp_focuser` and `third_powerbox` scenarios of `test_rotator_lunatico_simulator.c`. | Open (port-device lifecycle rework; deferred to the generator migration, see `focuser_lunatico/REFACTOR.md`) |
