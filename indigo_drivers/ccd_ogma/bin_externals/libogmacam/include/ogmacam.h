...
/*
    Coordinate:
        (a) Functions with coordinate parameters, such as Ogmacam_put_Roi, Ogmacam_put_AEAuxRect, etc., the coordinate is always relative to the original resolution,
...
#define OGMACAM_FLAG_40GIGE               0x0001000000000000  /* 40 Gigabit GigE */
...
#define OGMACAM_FLAG_USB32                0x0400000000000000  /* USB 3.2 Gen 2 */
#define OGMACAM_FLAG_USB32_OVER_USB30     0x0800000000000000  /* USB 3.2 Gen 2 camera connected to usb3.0 port */
#define OGMACAM_FLAG_LINESCAN             0x1000000000000000  /* line scan camera */
#define OGMACAM_FLAG_25GIGE               0x2000000000000000  /* 2.5 Gigabit GigE */
#define OGMACAM_FLAG_RAW14PACK            0x4000000000000000  /* pixel format, RAW 14bits packed */
...
#define OGMACAM_ANTIBLOOMING_MIN          0       /* Anti Blooming */
#define OGMACAM_GVCP_RETRY_DEF            4       /* GVCP Retry */
#define OGMACAM_GVCP_RETRY_MIN            2
#define OGMACAM_GVCP_RETRY_MAX            20
#define OGMACAM_GVCP_TIMEOUT_DEF          40      /* GVCP Timeout */
#define OGMACAM_GVCP_TIMEOUT_MIN          20
#define OGMACAM_GVCP_TIMEOUT_MAX          200
#define OGMACAM_GVSP_WAIT_PERCENT_DEF     1       /* GVSP wait percent */
#define OGMACAM_GVSP_WAIT_PERCENT_MIN     0
#define OGMACAM_GVSP_WAIT_PERCENT_MAX     100
#define OGMACAM_FRONTEND_MAX              1024    /* frontend frame buffer deque length */
#define OGMACAM_FRONTEND_DEF              4
#define OGMACAM_FRONTEND_MIN              2
#define OGMACAM_BACKEND_MAX               1024    /* backend frame buffer deque length */
#define OGMACAM_BACKEND_DEF               3
#define OGMACAM_BACKEND_MIN               2
#define OGMACAM_HEAT_MIN                  0       /* Heat */
#define OGMACAM_LANE_MIN                  0       /* Lane */
#define OGMACAM_FAN_MIN                   0       /* Fan */
...
#endif
    const OgmacamModelV2* model;              /* Functionally equivalent to a global constant, remaining valid and unchanged throughout the lifetime of the process */
...
*/
#if defined(_WIN32)
OGMACAM_API(HRESULT)  Ogmacam_Load(HOgmacam h, const wchar_t* strPara);
#else
OGMACAM_API(HRESULT)  Ogmacam_Load(HOgmacam h, const char* strPara);
#endif
...
#define OGMACAM_EVENT_FRONT_OVERFLOW    0x0010    /* front buffer overflow */
#define OGMACAM_EVENT_BACK_OVERFLOW     0x0011    /* back buffer overflow */
...
#define OGMACAM_FRAMEINFO_FLAG_HOB                0x00000400 /* Horizontal Optical Black */
#define OGMACAM_FRAMEINFO_FLAG_VOB                0x00000800 /* Vertical Optical Black */
...
*/
OGMACAM_API(HRESULT)  Ogmacam_get_AutoExpoEnable(HOgmacam h, int* mode);
OGMACAM_API(HRESULT)  Ogmacam_put_AutoExpoEnable(HOgmacam h, int mode);
...
OGMACAM_API(HRESULT)  Ogmacam_put_AWBAuxRect(HOgmacam h, const RECT* pRect); /* auto white balance ROI */
OGMACAM_API(HRESULT)  Ogmacam_get_AWBAuxRect(HOgmacam h, RECT* pRect);
OGMACAM_API(HRESULT)  Ogmacam_put_AEAuxRect(HOgmacam h, const RECT* pRect);  /* auto exposure ROI */
OGMACAM_API(HRESULT)  Ogmacam_get_AEAuxRect(HOgmacam h, RECT* pRect);

OGMACAM_API(HRESULT)  Ogmacam_put_ABBAuxRect(HOgmacam h, const RECT* pRect); /* auto black balance ROI */
OGMACAM_API(HRESULT)  Ogmacam_get_ABBAuxRect(HOgmacam h, RECT* pRect);
...
/* Case insensitive */
OGMACAM_API(HRESULT)  Ogmacam_put_Int(HOgmacam h, const char* strName, int iValue);
OGMACAM_API(HRESULT)  Ogmacam_get_Int(HOgmacam h, const char* strName, int* piValue);
OGMACAM_API(HRESULT)  Ogmacam_put_Str(HOgmacam h, const char* strName, const char* strValue);
/* strValue: output buffer (128 bytes are always sufficient) */
OGMACAM_API(HRESULT)  Ogmacam_get_Str(HOgmacam h, const char* strName, char strValue[]);
OGMACAM_API(HRESULT)  Ogmacam_get_Enum(HOgmacam h, const char* strName, int* pNumber, int arrValue[], const char* arrString[]);
OGMACAM_API(HRESULT)  Ogmacam_get_StrPtr(HOgmacam h, const char* strName, const char** strValue);
...
#define OGMACAM_OPTION_CALLBACK_THREAD        0x30       /* [RW] dedicated thread for callback: 0 => disable, 1 => enable
                                                                 default: 1
                                                         */
...
#define OGMACAM_OPTION_FPNC                   0x67       /* [RW] Fix Pattern Noise Correction
                                                             ...
                                                             get:
                                                                 ...
                                                                 ((val & 0xff0000) >> 16): average number
                                                         */
#define OGMACAM_OPTION_OVEREXP_POLICY         0x68       /* [RW] Auto exposure overexposure policy: when overexposed,
...
#define OGMACAM_OPTION_CDS                    0x6e       /* [RW] Correlated Double Sampling: 0~max (OGMACAM_OPTION_CDS_MAX)
...
#define OGMACAM_OPTION_GVCP_TIMEOUT           0x71       /* [RW] GVCP Timeout: millisecond, range = [5, 150], default: 15(wire), 30(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
#define OGMACAM_OPTION_GVCP_RETRY             0x72       /* [RW] GVCP Retry: range = [2, 16], default: 4(wire), 8(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
...
#define OGMACAM_OPTION_TIMED_TRIGGER_NUM      0x7c       /* [RW] Timed trigger number */
...
#define OGMACAM_OPTION_LANE                   0x7e       /* [RW] Lane */
...
#define OGMACAM_OPTION_SCANTYPE               0x8e       /* [RW] Scan Type: 0(areascan), 1(linescan) */
...
#define OGMACAM_PIXELFORMAT_RAW14PACK         0x14
...
/*
 ...
 unsigned index = 0;
 do {
     if (FAILED(Ogmacam_get_BinningMethod(h, index, &pStrMethod)))
         break;
     ++index;
 } while (1);
*/
...
#define OGMACAM_IOCONTROLTYPE_GET_PWM_FREQ                 0x19 /* PWM Frequency, range: [0, 0xffffffff] */
#define OGMACAM_IOCONTROLTYPE_SET_PWM_FREQ                 0x1a
#define OGMACAM_IOCONTROLTYPE_GET_PWM_DUTYRATIO            0x1b /* PWM Duty Ratio, default: 50, range: [0, 100] */
...
#define OGMACAM_IOCONTROLTYPE_GET_EXPO_START_LINE          0x31 /* exposure start line, default: 0, range: [0, 16384] */
#define OGMACAM_IOCONTROLTYPE_SET_EXPO_START_LINE          0x32
#define OGMACAM_IOCONTROLTYPE_GET_EXPO_END_LINE            0x33 /* exposure end line, default: 0, range: [0, 16384]
...
#define OGMACAM_IOCONTROLTYPE_GET_USER_PULSE_HIGH          0x3d /* User pulse high level time: us, range: [0, 0xffffffff] */
#define OGMACAM_IOCONTROLTYPE_SET_USER_PULSE_HIGH          0x3e
#define OGMACAM_IOCONTROLTYPE_GET_USER_PULSE_LOW           0x3f /* User pulse low level time: us, range: [0, 0xffffffff] */
#define OGMACAM_IOCONTROLTYPE_SET_USER_PULSE_LOW           0x40
#define OGMACAM_IOCONTROLTYPE_GET_USER_PULSE_NUMBER        0x41 /* User pulse number: default 0, range: [0, 0xffffffff] */
...
/* flash zone */
#define OGMACAM_FLASH_SENSOR    0x00    /* sensor */
#define OGMACAM_FLASH_USER      0x02    /* user */
/*
 action = (zone << 24) | OGMACAM_FLASH_XXXX: read, write, erase, query total size, query read/write block size, query erase block size
 addr = address
 see democpp
*/
...
*/
OGMACAM_API(HRESULT)  Ogmacam_GigeEnableV2(POGMACAM_HOTPLUG funHotPlug, void* ctxHotPlug, const char* opt);
...
OGMACAM_API(HRESULT) Ogmacam_readPtr(HOgmacam h, const char* key, int len, void* pData);
OGMACAM_API(HRESULT) Ogmacam_writePtr(HOgmacam h, const char* key, int len, const void* pData);
...
OGMACAM_API(HRESULT)  Ogmacam_query_SerialNumber(const wchar_t* camId, char sn[32]);
#else
...
OGMACAM_API(HRESULT)  Ogmacam_query_SerialNumber(const char* camId, char sn[32]);
#endif
