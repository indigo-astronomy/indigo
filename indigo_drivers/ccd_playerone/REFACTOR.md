...
| Images | `image_formats_roi_and_bins`, `bin_boundaries_and_raw16_only`, `geometry_errors_and_recovery`, `frame_types_and_exposure_units`: simulator RAW/RGB fixtures, all four SDK formats, RAW16-only mono capability, ROI and bin dimensions, alignment/bounds, setup/readback failures and recovery. RAW pixels, geometry and Bayer metadata are checked at the driver handoff; framework encoders are outside driver coverage. |
