...
Temma is the only driver that opens its port with parity (`19200-8E1`), and the Temma protocol needs it. A Linux PTY cannot carry parity: the kernel clears `PARENB` on every `TCSETS`, and glibc's `tcsetattr()` fails with `EINVAL` on a reopen, when the only requested change left is the parity the kernel drops again. The first open succeeds because the speed and modes change. `strace` shows `TCSETS` returning 0 followed by a `TCGETS` without `PARENB`, and a standalone open/configure/close loop on one PTY reproduces it: success, then `EINVAL`, `EINVAL`. macOS PTYs do not trip this and a real UART applies the parity, so this is a PTY limitation, not a driver defect. The shared I/O layer is left unchanged; instead the three cases that reopen the same simulator PTY are compiled out on Linux (`#ifndef __linux__` in `main()` of `test_mount_temma_simulator.c`), so the Linux suite runs 13 cases.
