...
## Linux simulator run (2026-09-26)

The portable suite passed 16/16 on macOS but failed 16/13 on Linux x64 (driver 3.0.0.16, reproduced unchanged at 3.0.0.18 on Linux x64 and Linux arm64 in Debian bookworm containers, glibc 2.36). The failing cases were `temma_park_abort_disconnect_and_read_failure`, `temma_timeout_and_open_failures_recover` and `temma_shared_lifecycle_and_pending_disconnect`, each with `Failed to open /dev/pts/N (Invalid argument)` when the driver reconnected to the PTY of a simulator it had already used.

Temma is the only driver that opens its port with parity (`19200-8E1`). A Linux PTY cannot carry parity: the kernel clears `PARENB` on every `TCSETS`, and glibc's `tcsetattr()` reads the settings back and fails with `EINVAL` when none of the requested changes took effect. The first open succeeds because the speed and modes change; every later open of the same PTY fails because they are already in place and only the dropped parity differs. `strace` shows `TCSETS` returning 0 followed by a `TCGETS` without `PARENB`, and a standalone open/configure/close loop on one PTY reproduces it: success, then `EINVAL`, `EINVAL`. macOS PTYs do not trip this, and a real UART applies the parity, so the defect needs a PTY (a simulator, or a `socat`/`ser2net` virtual port on a user's machine). The fix is in the shared I/O layer, not in the driver: on Linux `open_tty()` in `indigo_libs/indigo_uni_io.c` accepts an `EINVAL` from `tcsetattr()` when a read-back shows that everything except the character format (`CSIZE`, `PARENB`, `PARODD`) is in place, which is the state the first open already accepts. No driver change, so the driver version is unchanged.

A fourth, intermittent failure showed up in `temma_guider_timing_idle_and_mount_workload` once the reconnects worked (4 of 12 runs on Linux arm64): `measure_pulse()` read the simulator trace once, right after the guide property turned OK, and could miss the closing relay mask the driver had already written but the simulator had not recorded yet. It now polls the trace for up to a second, like `trace_contains_after()`. The measured pulse length comes from the simulator's receive timestamps and is not affected.

After both changes: Linux x64 5 full runs, 16/16 each; Linux arm64 5 full runs, 16/16 each.
