...
#define MALLINCAM_FLAG_40GIGE               0x0001000000000000  /* 40 Gigabit GigE */
...
#define MALLINCAM_FLAG_25GIGE               0x2000000000000000  /* 2.5 Gigabit GigE */
#define MALLINCAM_FLAG_RAW14PACK            0x4000000000000000  /* pixel format, RAW 14bits packed */
...
#define MALLINCAM_ANTIBLOOMING_MIN          0       /* Anti Blooming */
#define MALLINCAM_GVCP_RETRY_DEF            4       /* GVCP Retry */
#define MALLINCAM_GVCP_RETRY_MIN            2
#define MALLINCAM_GVCP_RETRY_MAX            20
#define MALLINCAM_GVCP_TIMEOUT_DEF          40      /* GVCP Timeout */
#define MALLINCAM_GVCP_TIMEOUT_MIN          20
#define MALLINCAM_GVCP_TIMEOUT_MAX          200
#define MALLINCAM_GVSP_WAIT_PERCENT_DEF     1       /* GVSP wait percent */
#define MALLINCAM_GVSP_WAIT_PERCENT_MIN     0
#define MALLINCAM_GVSP_WAIT_PERCENT_MAX     100
#define MALLINCAM_FRONTEND_MAX              1024    /* frontend frame buffer deque length */
#define MALLINCAM_FRONTEND_DEF              4
#define MALLINCAM_FRONTEND_MIN              2
#define MALLINCAM_BACKEND_MAX               1024    /* backend frame buffer deque length */
#define MALLINCAM_BACKEND_DEF               3
#define MALLINCAM_BACKEND_MIN               2
#define MALLINCAM_HEAT_MIN                  0       /* Heat */
#define MALLINCAM_LANE_MIN                  0       /* Lane */
#define MALLINCAM_FAN_MIN                   0       /* Fan */
...
#endif
    const MallincamModelV2* model;              /* Functionally equivalent to a global constant, remaining valid and unchanged throughout the lifetime of the process */
...
*/
#if defined(_WIN32)
MALLINCAM_API(HRESULT)  Mallincam_Load(HMallincam h, const wchar_t* strPara);
#else
MALLINCAM_API(HRESULT)  Mallincam_Load(HMallincam h, const char* strPara);
#endif
...
#define MALLINCAM_EVENT_FRONT_OVERFLOW    0x0010    /* front buffer overflow */
#define MALLINCAM_EVENT_BACK_OVERFLOW     0x0011    /* back buffer overflow */
...
#if defined(_WIN32)
MALLINCAM_API(HRESULT)  Mallincam_StartPullModeWithWndMsg(HMallincam h, HWND hWnd, unsigned msgWnd);
#endif
...
#define MALLINCAM_FRAMEINFO_FLAG_HOB                0x00000400 /* Horizontal Optical Black */
#define MALLINCAM_FRAMEINFO_FLAG_VOB                0x00000800 /* Vertical Optical Black */
...
typedef struct {
    ...
    unsigned char pixelFormat;      /* MALLINCAM_PIXELFORMAT_xxxx */
    unsigned char ergb;             /* see MALLINCAM_OPTION_RGB */
    unsigned char snapR;            /* see Mallincam_SnapR */
    unsigned char infoVer;          /* MallincamFrameInfo version >= 4 */
    unsigned reserved;
    unsigned strideRaw;             /* stride of RAW, 0 means = image width (no padding) */
    MallincamFrameInfoV4* ptrInfo;
    void* snapCtx;
    void* ptrRaw;                   /* RAW, see MALLINCAM_OPTION_IMAGEPTRRAW */
    unsigned char* ptr8;
...
MALLINCAM_API(HRESULT)  Mallincam_PullImagePtr(HMallincam h, int bStill, MallincamImagePtr* ptrImage);
...
/* After a frame buffer has been used, it must be returned to the SDK for reuse. Please note the following:
    ...
    (e) Frame buffers that have not been returned remain valid after Mallincam_Stop. Frame buffers that have not been returned become invalid immediately after Mallincam_Close.
*/
MALLINCAM_API(HRESULT)  Mallincam_PushImagePtr(HMallincam h, unsigned ptrId);
...
/* waitMS: The timeout interval, in milliseconds. If a nonzero value is specified, the function waits until the image is ok or the interval elapses.
            If waitMS is zero, the function does not enter a wait state if the image is not available; it always returns immediately; this is equal to Mallincam_PullImagePtr.
*/
MALLINCAM_API(HRESULT)  Mallincam_WaitImagePtr(HMallincam h, unsigned waitMS, int bStill, MallincamImagePtr* ptrImage);

/*
    waitMS: The timeout interval, in milliseconds. If a nonzero value is specified, the function waits until the image is ok or the interval elapses.
             If waitMS is zero, the function does not enter a wait state if the image is not available; it always returns immediately; this is equal to Mallincam_PullImageV4.
    bStill: to pull still image, set to 1, otherwise 0
...
MALLINCAM_API(HRESULT)  Mallincam_WaitImageV4(HMallincam h, unsigned waitMS, void* pImageData, int bStill, int bits, int rowPitch, MallincamFrameInfoV4* pInfo);
...
MALLINCAM_API(HRESULT)  Mallincam_WaitImageV3(HMallincam h, unsigned waitMS, void* pImageData, int bStill, int bits, int rowPitch, MallincamFrameInfoV3* pInfo);
...
MALLINCAM_API(HRESULT)  Mallincam_SnapV2(HMallincam h, unsigned nResolutionIndex, unsigned nNumber, int eRGB, void* snapCtx);
...
*/
MALLINCAM_API(HRESULT)  Mallincam_TriggerSyncV4(HMallincam h, unsigned waitMS, void* pImageData, int bits, int rowPitch, MallincamFrameInfoV4* pInfo);
MALLINCAM_API(HRESULT)  Mallincam_TriggerSync(HMallincam h, unsigned waitMS, void* pImageData, int bits, int rowPitch, MallincamFrameInfoV3* pInfo);
...
MALLINCAM_API(HRESULT)  Mallincam_put_AWBAuxRect(HMallincam h, const RECT* pRect); /* auto white balance ROI */
MALLINCAM_API(HRESULT)  Mallincam_get_AWBAuxRect(HMallincam h, RECT* pRect);
MALLINCAM_API(HRESULT)  Mallincam_put_AEAuxRect(HMallincam h, const RECT* pRect);  /* auto exposure ROI */
MALLINCAM_API(HRESULT)  Mallincam_get_AEAuxRect(HMallincam h, RECT* pRect);

MALLINCAM_API(HRESULT)  Mallincam_put_ABBAuxRect(HMallincam h, const RECT* pRect); /* auto black balance ROI */
MALLINCAM_API(HRESULT)  Mallincam_get_ABBAuxRect(HMallincam h, RECT* pRect);
...
/* Case insensitive */
MALLINCAM_API(HRESULT)  Mallincam_put_Int(HMallincam h, const char* strName, int iValue);
MALLINCAM_API(HRESULT)  Mallincam_get_Int(HMallincam h, const char* strName, int* piValue);
MALLINCAM_API(HRESULT)  Mallincam_put_Str(HMallincam h, const char* strName, const char* strValue);
/* strValue: output buffer (128 bytes are always sufficient) */
MALLINCAM_API(HRESULT)  Mallincam_get_Str(HMallincam h, const char* strName, char strValue[]);
MALLINCAM_API(HRESULT)  Mallincam_get_Enum(HMallincam h, const char* strName, int* pNumber, int arrValue[], const char* arrString[]);
MALLINCAM_API(HRESULT)  Mallincam_get_StrPtr(HMallincam h, const char* strName, const char** strValue);
...
#define MALLINCAM_OPTION_BLACKLEVEL             0x15       /* [RW] the black level refers to the baseline signal value output by an image sensor under no-light (completely dark) conditions.
...
#define MALLINCAM_OPTION_CALLBACK_THREAD        0x30       /* [RW] dedicated thread for callback: 0 => disable, 1 => enable
                                                                 default: 1
                                                         */
...
#define MALLINCAM_OPTION_FPNC                   0x67       /* [RW] Fix Pattern Noise Correction
                                                             ...
                                                             get:
                                                                 ...
                                                                 ((val & 0xff0000) >> 16): average number
                                                         */
#define MALLINCAM_OPTION_OVEREXP_POLICY         0x68       /* [RW] Auto exposure overexposure policy: when overexposed,
...
#define MALLINCAM_OPTION_CDS                    0x6e       /* [RW] Correlated Double Sampling: 0~max (MALLINCAM_OPTION_CDS_MAX)
...
#define MALLINCAM_OPTION_GVCP_TIMEOUT           0x71       /* [RW] GVCP Timeout: millisecond, range = [5, 150], default: 15(wire), 30(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
#define MALLINCAM_OPTION_GVCP_RETRY             0x72       /* [RW] GVCP Retry: range = [2, 16], default: 4(wire), 8(wireless)
                                                              Unless in very special circumstances, generally no modification is required, just use the default value
                                                         */
...
#define MALLINCAM_OPTION_TIMED_TRIGGER_NUM      0x7c       /* [RW] Timed trigger number */
...
#define MALLINCAM_OPTION_LANE                   0x7e       /* [RW] Lane */
...
#define MALLINCAM_OPTION_ANTI_BLOOMING          0x8b       /* [RW] Anti Blooming, maximum
                                                         ...
                                                         */
#define MALLINCAM_OPTION_ANTI_BLOOMING_MAX      0x8c       /* [RO] Anti Blooming */
#define MALLINCAM_OPTION_CDS_MAX                0x8d       /* [RO] Correlated Double Sampling */
#define MALLINCAM_OPTION_SCANTYPE               0x8e       /* [RW] Scan Type: 0(areascan), 1(linescan) */
#define MALLINCAM_OPTION_OPERATIONMODE          0x8f       /* [RW] TDI Operation Mode: 1(area), 2(TDI) */
#define MALLINCAM_OPTION_TDITRIGGERMODE         0x90       /* [RW] TDI Trigger Mode: 1(normal), 2(both) */
#define MALLINCAM_OPTION_TDISTAGE               0x91       /* [RW] TDI Trigger Stage: sensor scan stage */
#define MALLINCAM_OPTION_FRAMEINTERVAL          0x92       /* [RW] Frame Interval in microseconds */
#define MALLINCAM_OPTION_FRAMEINTERVAL_MIN      0x93       /* [RO] Frame Interval, minimum */
#define MALLINCAM_OPTION_FRAMEINTERVAL_MAX      0x94       /* [RO] Frame Interval, maximum */
#define MALLINCAM_OPTION_IMAGEPTRRAW            0x95       /* [RW] default: 0 */
#define MALLINCAM_OPTION_IMAGEPTRBOTH           0x96       /* [RW] default: 0 */
...
#define MALLINCAM_PIXELFORMAT_RAW14PACK         0x14
...
MALLINCAM_API(int)         Mallincam_get_PixelFormatBitdepth(int pixelFormat);
...
/*
 ...
 unsigned index = 0;
 do {
     if (FAILED(Mallincam_get_BinningMethod(h, index, &pStrMethod)))
         break;
     ++index;
 } while (1);
*/
...
#define MALLINCAM_IOCONTROLTYPE_GET_PWM_FREQ                 0x19 /* PWM Frequency, range: [0, 0xffffffff] */
#define MALLINCAM_IOCONTROLTYPE_SET_PWM_FREQ                 0x1a
#define MALLINCAM_IOCONTROLTYPE_GET_PWM_DUTYRATIO            0x1b /* PWM Duty Ratio, default: 50, range: [0, 100] */
...
#define MALLINCAM_IOCONTROLTYPE_GET_EXPO_START_LINE          0x31 /* exposure start line, default: 0, range: [0, 16384] */
#define MALLINCAM_IOCONTROLTYPE_SET_EXPO_START_LINE          0x32
#define MALLINCAM_IOCONTROLTYPE_GET_EXPO_END_LINE            0x33 /* exposure end line, default: 0, range: [0, 16384]
...
#define MALLINCAM_IOCONTROLTYPE_GET_USER_PULSE_HIGH          0x3d /* User pulse high level time: us, range: [0, 0xffffffff] */
#define MALLINCAM_IOCONTROLTYPE_SET_USER_PULSE_HIGH          0x3e
#define MALLINCAM_IOCONTROLTYPE_GET_USER_PULSE_LOW           0x3f /* User pulse low level time: us, range: [0, 0xffffffff] */
#define MALLINCAM_IOCONTROLTYPE_SET_USER_PULSE_LOW           0x40
#define MALLINCAM_IOCONTROLTYPE_GET_USER_PULSE_NUMBER        0x41 /* User pulse number: default 0, range: [0, 0xffffffff] */
...
/* flash zone */
#define MALLINCAM_FLASH_SENSOR    0x00    /* sensor */
#define MALLINCAM_FLASH_USER      0x02    /* user */
/*
 action = (zone << 24) | MALLINCAM_FLASH_XXXX: read, write, erase, query total size, query read/write block size, query erase block size
 addr = address
 see democpp
*/
...
MALLINCAM_API(HRESULT) Mallincam_readPtr(HMallincam h, const char* key, int len, void* pData);
MALLINCAM_API(HRESULT) Mallincam_writePtr(HMallincam h, const char* key, int len, const void* pData);
...
MALLINCAM_API(HRESULT) Mallincam_Reset(const wchar_t* camId);
...
MALLINCAM_API(HRESULT) Mallincam_Reset(const char* camId);
...
MALLINCAM_API(HRESULT)  Mallincam_query_SerialNumber(const wchar_t* camId, char sn[32]);
#else
...
MALLINCAM_API(HRESULT)  Mallincam_query_SerialNumber(const char* camId, char sn[32]);
#endif
