...
ORIGINAL_POSITION=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_POSITION.POSITION" 2>&1)
echo "Original position: $ORIGINAL_POSITION"

# Store original ON_POSITION_SET mode
ORIGINAL_GOTO=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_ON_POSITION_SET.GOTO" 2>&1)
ORIGINAL_SYNC=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_ON_POSITION_SET.SYNC" 2>&1)

# Store optional properties if they exist
if property_exists "$DEVICE.FOCUSER_REVERSE_MOTION"; then
	ORIGINAL_REVERSE_ENABLED=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_REVERSE_MOTION.ENABLED" 2>&1)
	ORIGINAL_REVERSE_DISABLED=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_REVERSE_MOTION.DISABLED" 2>&1)
...
if property_exists "$DEVICE.FOCUSER_BACKLASH"; then
	ORIGINAL_BACKLASH=$($INDIGO_PROP_TOOL get -w OK $REMOTE_SERVER "$DEVICE.FOCUSER_BACKLASH.BACKLASH" 2>&1)
