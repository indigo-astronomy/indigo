...
error:
    ...
    while (!STACK_EMPTY(context, tag_directives_copy)) {
        yaml_tag_directive_t value1 = POP(context, tag_directives_copy);
        yaml_free(value1.handle);
        yaml_free(value1.prefix);
    }
...
error:
    ...
    yaml_free(version_directive_copy);
    while (!STACK_EMPTY(&context, tag_directives_copy)) {
        yaml_tag_directive_t value2 = POP(&context, tag_directives_copy);
        yaml_free(value2.handle);
        yaml_free(value2.prefix);
    }
    STACK_DEL(&context, tag_directives_copy);
    yaml_free(value.handle);
    yaml_free(value.prefix);
