 ...
 */
unsigned int IPAManager::addDir()
{
	std::vector<std::string> files{};
	// As of now, we are only supporting RPi 4
	//
	files.push_back("rpi/vc4");
	// files.push_back("rpi/pisp");
 ...
 */
IPAModule *IPAManager::module(PipelineHandler *pipe)
{
	if (modules_.empty()) {
		LOG(IPAManager, Error) << "No IPA modules loaded";
		return nullptr;
	}
	std::cout << "modules__.size() = " << modules_.size() << std::endl;
	for (const auto &module : modules_) {
		
		if (module->match(pipe)){
				std::cout << "IPAManager::module..." << std::endl;
				return module.get();
			}
	 }
