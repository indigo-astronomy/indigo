...
#include <yaml.h>
// Read values from data map instead of json files
//
#include "libcamera/internal/json_data_registry.h"
/**
 * \file yaml_parser.h
...
int YamlParserContext::init(std::string &file)
{
	/* yaml_parser_initialize returns 1 when it succeededs */
	if (!yaml_parser_initialize(&parser_)) {
		LOG(YamlParser, Error) << "Failed to initialize YAML parser";
	...
	JsonData *pData = get_json_data(file);
	parserValid_ = true;
	yaml_parser_set_input_string(&parser_, pData->data, pData->length);
 ...
 */
std::unique_ptr<YamlObject> YamlParser::parse(std::string &file)
{
