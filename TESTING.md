...
### Atik One legacy `ccd_atik2` acceptance (2026-09-12, 22:31–23:01)

macOS arm64, generated `ccd_atik2` version 9, bundled legacy `libatik` 1.9 (`Darwin/x64` identifier), USB `20e7:df3c`, serial `0869558227`, camera firmware `0.40`, RAW16 `3379 x 2703`, integrated five-slot wheel and adjustable cooling. The camera exposed no guider and the library flags exposed no shutter. The first sandboxed process found no USB camera and exited before connection; subsequent explicitly authorized hardware processes ran outside the sandbox.

The first complete version-8 run passed the CCD, geometry, read-mode and cooling phases but rejected the first wheel move because bundled `ic24_set_filter_wheel()` reports false even after sending a successful command. Arm64 disassembly confirmed that the function has no true-return path. Version 9 therefore determines wheel completion from bounded position readback. A diagnostic run then established that readback position `0` is the library's moving sentinel; the driver now accepts zero only during motion while retaining the last valid public slot. The isolated wheel run passed `1 -> 2 -> 3 -> 4 -> 5 -> 1` and restored slot 1.

The first complete version-9 run reached all functional phases but the shared hardware harness restored its original FITS format after geometry, then incorrectly counted later valid FITS images as invalid RAW. The harness now retains its explicitly selected RAW test format until cleanup and reports format/signature details for invalid images. Isolated geometry, read-mode, cooling and abort/restart runs all passed before the corrected complete rerun.

The final complete run passed. Requested exposures `.001/.1/1.5/2.5/16.5` seconds completed in `10.823/10.928/12.654/13.675/27.663` seconds including full-frame readout. All five frame types, `(16,16) 128 x 128` ROI, every advertised bin from 1 through 128, HIGH_SPEED and LOW_NOISE, cooling `21.9 -> 18.9 °C` with nonzero power and a valid cooled image, all wheel slots and restoration, abort/restart, disconnect/reconnect, driver shutdown, `dlclose/dlopen` and a fresh exposure passed. Cleanup restored wheel slot 1, the original cooler selection, temperature target and camera properties, then disconnected and shut down cleanly. Authoritative log: `/private/tmp/atik-one-ccd-atik2-v9-full-final.log`.

All 24 fake-SDK groups subsequently passed in the normal universal and arm64 ASan/UBSan builds, and generated output remained reproducible. Physical hot-plug was not performed for `ccd_atik2`; guider/electrical timing, optical image quality, shutter behavior, cooling accuracy/full warm-up, multiple cameras, other Atik models and non-macOS platforms remain unverified. Test binaries were cleaned and all owned processes ended. No README was changed.
