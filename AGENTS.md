...
- After completing any driver migration, update the driver's row in root `MIGRATION_STATUS.md` in the same change. Record the actual API, generator and async-queue status, automated-test availability and performed retesting; distinguish simulator validation from hardware validation and note unverified platforms or remaining limitations. Keep the driver's `REFACTOR.md` results consistent with this status.
