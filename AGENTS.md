...
- For inherited properties, prefer generator attributes such as `hidden`, `persistent`, `handle_change`, `asynchronous_change`, and `preserve_values` over hand-written attach/change boilerplate. Remember that `MOUNT_EQUATORIAL_COORDINATES` is finalized by the generator with `indigo_update_coordinates()`.
