...
`INDIGO_DRIVER_SHUTDOWN` in both Bluetooth focuser drivers set `delegate = nil`
without tearing down any CoreBluetooth state. If a STACKRAIL or HC-08 peripheral
was connected or being scanned for, the delegate object's reference to the
`CBCentralManager` kept delivering callbacks to the (imminently freed) object, and
the malloc-backed INDIGO `device` and `private_data` were never released.

The fix adds `-(void)shutdown` to both `MJKZZBTDelegate` and `WeMacroBTDelegate`.
The method:
1. Sets `central.delegate = nil` first, so no further CoreBluetooth callbacks fire.
2. Stops an active scan and clears the `scanning` flag (under `@synchronized`).
3. Calls `cancelPeripheralConnection` if the peripheral is not already disconnected.
4. Calls `[self deleteDevice]`, which calls `indigo_detach_device`, frees the
   malloc'd `device` and `private_data`, and zeroes the pointers.

`INDIGO_DRIVER_SHUTDOWN` now calls `[delegate shutdown]` before `delegate = nil`
