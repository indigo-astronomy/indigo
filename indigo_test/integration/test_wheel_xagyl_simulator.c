...
static void xagyl_wheel_passes_serial_compliance_checks(void) {
	...
	SERIAL_CHECK_TRUE(start_external_serial_simulator(&simulator, WHEEL_XAGYL_SIMULATOR_EXECUTABLE));
	...
	SERIAL_CHECK_EQ_INT(INDIGO_OK, indigo_change_number_property_1(&simulator_test_client, xagyl_wheel.device_name, WHEEL_SLOT_PROPERTY_NAME, WHEEL_SLOT_ITEM_NAME, 3));
	...
	SERIAL_CHECK_EQ_INT(INDIGO_OK, indigo_change_number_property_1(&simulator_test_client, xagyl_wheel.device_name, WHEEL_SLOT_PROPERTY_NAME, WHEEL_SLOT_ITEM_NAME, 5));
	SERIAL_CHECK_TRUE(wait_for_property_state(WHEEL_SLOT_PROPERTY_NAME, INDIGO_BUSY_STATE));
	SERIAL_CHECK_TRUE(wait_for_property_state(WHEEL_SLOT_PROPERTY_NAME, INDIGO_OK_STATE));
...
cleanup:
	if (context.connected) {
		stop_serial_driver(&xagyl_wheel);
	}
...
int main(void) {
	const indigo_test_case tests[] = {
		{ "xagyl_wheel_passes_serial_compliance_checks", xagyl_wheel_passes_serial_compliance_checks }
	};
