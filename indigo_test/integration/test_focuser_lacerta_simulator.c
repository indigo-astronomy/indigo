...
static void lacerta_focuser_passes_serial_compliance_checks(void) {
	...
	SERIAL_CHECK_TRUE(start_external_serial_simulator(&simulator, FOCUSER_LACERTA_SIMULATOR_EXECUTABLE));
	SERIAL_CHECK_TRUE(start_serial_driver(&lacerta_focuser, simulator.port));
...
cleanup:
	if (context.connected) {
		stop_serial_driver(&lacerta_focuser);
	}
...
int main(void) {
	const indigo_test_case tests[] = {
		{ "lacerta_focuser_passes_serial_compliance_checks", lacerta_focuser_passes_serial_compliance_checks }
	};
	return indigo_run_tests("LACERTA Motorfocus serial simulator integration tests", tests, ARRAY_SIZE(tests));
}
