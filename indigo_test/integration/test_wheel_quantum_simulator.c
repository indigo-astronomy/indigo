...
static const simulator_driver_case quantum_wheel = {
	"Brightstar Quantum Filter Wheel",
...
static void quantum_wheel_passes_serial_compliance_checks(void) {
	...
	SERIAL_CHECK_TRUE(start_external_serial_simulator(&simulator, WHEEL_QUANTUM_SIMULATOR_EXECUTABLE));
	...
	SERIAL_CHECK_EQ_INT(INDIGO_OK, indigo_change_number_property_1(&simulator_test_client, quantum_wheel.device_name, WHEEL_SLOT_PROPERTY_NAME, WHEEL_SLOT_ITEM_NAME, 4));
	...
	SERIAL_CHECK_EQ_INT(INDIGO_OK, indigo_change_number_property_1(&simulator_test_client, quantum_wheel.device_name, WHEEL_SLOT_PROPERTY_NAME, WHEEL_SLOT_ITEM_NAME, 7));
	SERIAL_CHECK_TRUE(wait_for_property_state(WHEEL_SLOT_PROPERTY_NAME, INDIGO_OK_STATE));
	SERIAL_CHECK_TRUE(wait_for_number_item_value(WHEEL_SLOT_PROPERTY_NAME, WHEEL_SLOT_ITEM_NAME, 7, 0.001));

cleanup:
	if (context.connected) {
		stop_serial_driver(&quantum_wheel);
	}
...
int main(void) {
	const indigo_test_case tests[] = {
		{ "quantum_wheel_passes_serial_compliance_checks", quantum_wheel_passes_serial_compliance_checks }
	};
	return indigo_run_tests("Brightstar Quantum wheel serial simulator integration tests", tests, ARRAY_SIZE(tests));
}
