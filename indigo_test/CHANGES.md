...
## Generator LGPL propagation (2026-09-12)

Extended `integration/test_generator_architecture.c` with a split-line GNU Lesser General Public License fixture. The regression verifies LGPL 2.1-or-later propagation to generated implementation, public header and standalone main sources, preservation through reverse extraction, and the unchanged INDIGO license default for definitions without an LGPL notice.
...
Two new cases bring the suite to 77. `calibration single pulse speed` verifies 40 px/s RA calibration with exactly one west measurement pulse and one east return pulse (plus one backlash-clear pulse), without DEC commands. `calibration directional speed` gives west/east different physical responses (10/20 px/s), verifies their 15 px/s mean and equal measurement/return counts. The existing `calibration speed accuracy` now also checks zero measured backlash for the model without backlash. Other fixtures retain equal direction responses by default.

Validation: driver build passed. Six focused cases passed: `calibration speed accuracy`, `calibration single pulse speed`, `calibration directional speed`, `calibration adaptive step`, `calibration ra only`, and `calibration and guiding`, including cleanup. The full suite and AddressSanitizer were not rerun for this counting/arithmetic fix. DRV-130–DRV-133 remain open. Test artifacts were removed with `make -C indigo_test test-clean`.
...
Four new cases bring the suite to 70: `shutdown subframe` checks restoration and fresh guiding after reload; `shutdown exposure` checks a 30-second exposure is aborted during teardown within five seconds; `shutdown instances` shuts down an active primary guider and secondary preview then restarts; `remove active instance` verifies the secondary disappears while primary guiding continues and a replacement instance can attach. Existing `shutdown active`, `additional instances lifecycle`, `configuration reload` and `simultaneous agents` cover the remaining affected paths.

Validation: driver build passed. The `shutdown` selection passed 4/4, `instance` passed 3/3 (including the overlapping shutdown-instances case), and configuration reload/simultaneous agents each passed 1/1, both normally and with AddressSanitizer. This covers eight distinct scenarios and includes cleanup. Sanitizer build: `TEST_BUILD=build/drv125-asan GUIDER_TEST_FLAGS='-fsanitize=address -fno-omit-frame-pointer -O1'`. The prior `indigo_restore_switch_state()` use-after-free was not reported. The full agent suite was not rerun; DRV-126–DRV-133 remain open. Test artifacts were removed with `make -C indigo_test test-clean`.
...
Inventory: 150 modules — 5 Complete, 47 Partial, 52 Not audited, 46 No tests.
...
| `ccd_atik` | Generated | Fake SDK/USB | Complete | 40 scenario groups passed; Titan and One feature scenarios passed; Titan idle replug crashed once; One idle/active hot-plug passed; 11000A/Horizon pending. See Atik acceptance below. |
| `ccd_atik2` | Generated | Fake SDK/USB + Atik One | Complete | 24 hardware-free scenario groups pass; complete Atik One CCD/cooling/wheel acceptance passed on macOS, with documented remaining gaps. |
...
| `focuser_lakeside` | Generated | PTY/protocol simulator | Complete | Full applicable relative-only matrix: 33 isolated protocol, capability, motion, control, fault, lifecycle and instance scenarios. Hardware wire assumptions remain explicit. |
...
| `focuser_mjkzz` | Generated | PTY/protocol simulator | Complete | Full applicable absolute/relative matrix: 36 isolated protocol, capability, motion, speed, fault, lifecycle and instance scenarios. Hardware behavior remains explicitly unverified. |
...
| `focuser_moonlite` | Generated | PTY/protocol simulator | Complete | Full applicable absolute/relative, settings, temperature, fault, lifecycle and instance matrix: 38 isolated scenarios. Hardware behavior remains explicitly unverified. |
...
| `focuser_nfocus` | Generated | PTY/protocol simulator | Complete | Full applicable relative-only matrix: 10 named scenarios cover direct protocol framing, capability inventory, optional temperature, startup validation/retry, speed, relative motion, busy guard, abort, status failure recovery and additional instances. Hardware behavior remains explicitly unverified. |
| `focuser_nstep` | Generated | PTY/protocol simulator | Complete | Full applicable matrix: 12 named scenarios cover direct protocol framing, capability inventory (including `X_` stepping-mode/phase-wiring and read-only position), optional temperature/compensation/mode, startup validation/retry, speed, stepping mode/phase wiring, backlash/compensation/mode settings, relative motion with absolute position readback, busy guard, abort, status failure recovery and additional instances. Hardware behavior remains explicitly unverified. |
| `focuser_optec` | Generated | PTY/protocol simulator | Complete | Full applicable relative-only matrix: 32 isolated protocol, capability, motion, compensation, mode, fault, lifecycle and instance scenarios. Hardware behavior remains explicitly unverified. |
| `focuser_optecfl` | Hand-written | PTY/protocol simulator | Complete | Full applicable absolute-focuser matrix: 44 isolated lifecycle, property-contract, motion, sync, abort, settings, fault, shared-handle and temperature scenarios across both logical focusers. Generator migration explicitly not requested; hardware behavior remains unverified. |
...
| `focuser_robofocus` | Generated | PTY/protocol simulator | Complete | Full applicable fixed-frame protocol, absolute/relative motion, GOTO/SYNC, abort, settings, fault, lifecycle and instance matrix: 18 isolated scenarios. Hardware behavior remains explicitly unverified. |
| `focuser_steeldrive2` | Generated | PTY/protocol simulator | Complete | 49 isolated protocol, focuser, AUX, fault, transactional-init, shared-lifecycle and instance scenarios. Hardware behavior and Windows runtime remain explicitly unverified. |
...
| `focuser_wemacro` | Generated | PTY/protocol simulator | Complete | Full applicable relative-rail and batch matrix: 22 isolated protocol, capability, initialization, motion, control, fault, lifecycle and instance scenarios. Wire bytes remain implementation-derived; hardware and Windows runtime are unverified. |
...
- `test_focuser_optecfl_simulator.c`: 44 fork-isolated scenarios against a fresh Optec FocusLynx simulator each, driven by a per-command fault-injection file and a command journal. Covers driver metadata and interface bits; connect/disconnect and repeated INIT/SHUTDOWN for both logical focusers in either order; the shared hub handle opened once and released only on the last disconnect; property contract before, during and after connection; the complete Appendix A device-type table; non-default type readback; absolute GOTO with measured progress, no-op, boundaries, relative steps inward/outward, driver-resolved reversal, clamping, overlap rejection and externally observed motion; SYNC refused for homing device types and accepted otherwise; abort during motion, while idle, on failure and followed by a fresh move; device-type set and rejection; hub-info and configuration connect failures with descriptor accounting; a failed second connection leaving its sibling operational; handle release after sustained polling; malformed, partial, overlong, missing-acknowledgement, silent and split replies; poll-failure recovery; disconnect during motion; transport loss; and temperature polling with and without a probe.
...
- `test_focuser_lakeside_simulator.c`: 33 isolated pseudo-terminal scenarios with child watchdog/reaping and fresh property revisions. Direct simulator cases cover strict nominal/split command framing and elapsed movement. Driver cases cover pre/post-connect capabilities and ranges; inward/outward/no-op/end-clipped motion with BUSY/progress/measured completion; overlap; idle/moving abort and stable-position verification; stall, malformed progress/completion, start transport loss and stop failure; backlash, manual/automatic mode and both complete compensation profiles with ACK/read recovery; malformed/silent/partial/overlong initialization and polling with descriptor rollback/retry; external position/signed half-degree temperature; disconnect/reconnect and independent additional instances. Absolute GOTO/SYNC, exposed limits/calibration, speed and step-size configuration are non-applicable.
- `test_focuser_mjkzz_simulator.c`: 36 isolated pseudo-terminal scenarios with child watchdog/reaping and fresh property revisions. Direct simulator cases cover documented eight-byte big-endian signed framing, split requests/replies, checksum, address, ACK command bit, index, the complete command inventory (camera/focus acknowledgements, registers, all configuration/sequence setters and readbacks), speed, elapsed absolute/relative motion and STOP readback, plus rejection of bad checksum/address/command/index, invalid power and broadcasts. Driver cases cover pre/post-connect capabilities, permissions/ranges and initial readback; positive/negative/no-op/boundary absolute GOTO; inward/outward/zero/end-clipped relative motion with BUSY/progress/measured completion; speed 0..3 with ACK/readback; overlap; idle/moving abort and recovery; transactional failure/retry for version, all three startup registers, position and speed; corrupt/silent/partial/overlong response frames; idle polling and external signed position; start/read/stall/STOP failures; transport loss; disconnect during motion, missing-port recovery, repeated reconnect and independent additional instances. SYNC, reverse motion, temperature, backlash, limits, compensation, mode and camera/sequence functions are non-applicable at the driver property boundary because the driver does not expose them.
- `test_focuser_moonlite_simulator.c`: 38 isolated pseudo-terminal scenarios with child watchdog/reaping and fresh property revisions. Direct simulator cases cover strict nominal/split framing, temperature-conversion readiness, signed values, both motor ports, all documented driver commands and expanded controller settings, elapsed motion/stop, and malformed/unknown requests. Driver cases cover pre/post-connect properties, permissions/ranges and normal/alternate readback; speed, stepping, compensation, manual/automatic mode, reverse and software limits; zero/no-op/endpoint absolute GOTO and inward/outward/reversed/limited relative motion with BUSY/progress/measured completion; overlap; idle/moving abort; transactional probe/stop-status/position/coefficient/speed/step/temperature failures and same-process retry; silent/malformed/partial/overlong polling; setting, target, start, motion/status, stall and stop failures with recovery; signed temperature and external position; disconnect during motion, missing-port/repeated reconnect and independent additional instances. SYNC, backlash, homing, display controls, motor scaling and the second motor remain non-applicable at the INDIGO property boundary.
- `test_focuser_nfocus_simulator.c`: 10 isolated pseudo-terminal scenarios with fresh property revisions and command journal assertions. Direct simulator cases cover fixed-length identity, temperature, speed, startup-speed and status replies plus elapsed stop/status behavior. Driver cases cover pre/post-connect capability inventory, speed/steps ranges, hidden non-applicable position/reverse/limits/backlash/compensation/mode properties, optional absent temperature, transactional identity and speed-read startup failure/retry, `:CF` speed command spelling, inward/outward/zero relative moves, busy guarded overlap, moving abort, sticky malformed status recovery, reconnect after startup failure and independent additional instances. Absolute GOTO/SYNC, real position readback, hardware travel limits, stored-position UI behavior and hardware temperature accuracy are non-applicable or hardware-only.
- `test_focuser_nstep_simulator.c`: 12 isolated pseudo-terminal scenarios with fresh property revisions and command-journal assertions. Direct simulator cases cover fixed-length identity, temperature, speed, position, phase-wiring and backlash replies plus elapsed motion, absolute position readback and stop. Driver cases cover pre/post-connect capability inventory (speed/steps/backlash ranges, read-only `FOCUSER_POSITION`, `X_FOCUSER_STEPPING_MODE`/`X_FOCUSER_PHASE_WIRING`, hidden `FOCUSER_REVERSE_MOTION`), optional absent temperature hiding compensation/mode, transactional identity and position-read startup failure/retry, `:CO` speed command spelling and readback, stepping-mode copy-only versus `:CW` phase-wiring commands, `:TB`/`:TT`/`:TS`/`:TA`/`:TC` settings commands, inward/outward/zero relative moves with `:RP` position readback, busy guarded overlap, moving abort via `:F10000#`, sticky malformed status recovery and independent additional instances. Hardware travel, stored-position UI behavior and hardware temperature accuracy are non-applicable or hardware-only.
- `test_focuser_optec_simulator.c`: 32 isolated pseudo-terminal scenarios with child watchdog/reaping and fresh property revisions. Direct simulator cases cover normal/split unterminated commands, exact LF/CR replies, elapsed automatic compensation, TCF-S travel limits, invalid grammar/mode rejection and the documented manual, AUTO-A/AUTO-B, relative movement, position, temperature, A/B coefficient/sign/delay, quiet, center, sleep/wake, home and free-mode inventory. Driver cases cover pre/post-connect capabilities, hidden non-applicable properties, permissions/ranges and normal/alternate readback; inward/outward/reversed/zero/end-clipped relative motion with BUSY/progress/measured completion and overlap rejection; positive/negative/zero compensation with exact three-digit magnitude and partial-write reconciliation; manual/AUTO-A visibility and failure recovery; transactional handshake/position/temperature/coefficient/sign initialization, malformed/error/silent/partial/overlong framing, polling recovery, start/poll/stall/transport failures, disconnect during motion, missing-port/repeated reconnect, descriptor cleanup and independent additional instances. Absolute GOTO/SYNC, speed, backlash, limits and abort are non-applicable because the driver/protocol does not expose them; hardware travel, temperature accuracy and physical direction remain hardware-only.
...
- `test_focuser_robofocus_simulator.c`: 18 fork-isolated pseudo-terminal scenarios with child watchdog/reaping and fresh property revisions. Direct protocol coverage verifies nine-byte framing/checksums, invalid-checksum silence, firmware/position/sync/maximum/power/raw-configuration/backlash/temperature commands, elapsed motion and one progress tick per step. Driver cases cover pre/post-connect properties, ranges and normal/alternate/split readback; absolute GOTO and SYNC; inward/outward/reversed/no-op/limit-clipped relative motion with BUSY/progress/final readback and overlap rejection; idle/moving abort and recovery; all four power channels, raw duty/delay/step size and signed/zero backlash with partial failure reconciliation; transactional firmware/configuration/temperature initialization failure and same-process retry; polling and final-frame checksum failures; stalled motion; external position/temperature, disconnect/reconnect, disconnect during motion and independent additional instances. Speed control and temperature compensation are non-applicable because the driver does not expose them; hardware timing, travel, temperature, outputs and platform runtime remain unverified.
- `test_focuser_steeldrive2_simulator.c`: 49 fork-isolated scenarios against a fresh SteelDrive II simulator with watchdogs, fresh property revisions, command journal and one-shot faults. Direct protocol coverage verifies 19200-baud CR+LF framing, exact echo, Dallas/Maxim CRC8 enable/required/invalid-ignore/disable exceptions, unknown commands, GO/SUMMARY/STOP, zeroing, PID/PWM coupling, auto-dew and reboot. Focuser cases cover pre/connected property visibility, normal/alternate/split/missing-sensor readback, GOTO/SYNC, inward/outward/reversed/no-op/limit-clipped motion with BUSY progress and measured completion, overlap rejection, idle/moving abort, disconnect/reconnect during motion, both zeroing modes, name/saved fractional offsets/all compensation fields/sensor/end-stop controls, external position/temperature, setting/poll/start/stall recovery, RESET/REBOOT and every required initialization command with descriptor rollback and same-process retry. AUX cases cover normal/alternate/split readback, heater PWM, PID enable/target/fractional dew offset/sensors, automatic dew, PWM-disables-PID reconciliation, external PWM, setting failures and every required initialization command. Shared cases cover either logical-device connection order, one physical open, survivor operation after sibling disconnect and safe rejection of RESET with a sibling connected; a separate case verifies independent additional instances. TCOMP_PAUSE, expert motor-current/RCX controls and hardware electrical/mechanical/thermal behavior are non-applicable or hardware-only because the existing INDIGO property surface does not expose them.
...
- `test_focuser_wemacro_simulator.c`: 22 fork-isolated scenarios with fresh property revisions, bounded waits, child watchdogs, command/event journal and deterministic status/transport faults. Direct protocol coverage verifies exact 12-byte A5/5A framing, big-endian 32-bit payloads, little-endian Modbus/IBM CRC16, both directional completion states, speed/configuration flags, shutter, batch progress/return and rejection of short/header-invalid frames, bad CRC, fields, ranges and commands. Driver cases cover pre/connected property visibility and all published ranges; normal/split/fallback initialization; silent-start transactional rollback and same-process retry; both speeds, directions, local reverse, zero/no-op and relative BUSY/completion; overlap, active/idle abort, wrong/malformed/silent status recovery, active transport loss and disconnect/reconnect; BACK/BEEP and shutter; complete batch encoding, delayed two-stage start, no-return/return progress without target destruction, overlap/abort, second-write, wrong/malformed/silent status, active disconnect/reconnect and recovery; connected shutdown refusal and an independent additional instance. Absolute position/GOTO/SYNC, limits, backlash, temperature, compensation and automatic mode are non-applicable because the relative-only protocol/property surface does not expose them. Manufacturer pages do not publish the binary wire protocol, so physical travel/direction, shutter electrical timing, CH341/Bluetooth behavior, hardware concurrency and platform runtime remain unverified.
...
### DRV-139 — reject incomplete LX200 frames (2026-09-10)

Expanded `lx200 truncated command` covers EOF and read errors after a colon, queries, RA/DEC setters and slew/sync commands (12 fixtures); partial frames produce no reply or target update, close exactly once and preserve previously accepted requested coordinates. It also checks a complete query followed by a partial setter in one connection. New `lx200 command length` covers 127/128/129-byte payloads, an oversized setter followed by a valid command, unchanged coordinates and recovery on a new connection. Both cases failed against the pre-fix worker and pass with explicit terminator-gated dispatch.

Validation: these two regressions and `lx200 ack`, `lx200 positive zero`, `LX200 protocol`, `lx200 input matrix` passed (6/6), including cleanup. Production Mount Agent and the test executable built for macOS arm64/x86_64; tests ran on arm64 with scripted I/O and no hardware/network sockets. Version remains 0x03000016 per the user's deferred-version instruction. Test build artifacts removed with `make -C indigo_test test-clean`. DRV-140 malformed-coordinate handling is outside this fix.

### DRV-140 — validate LX200 coordinate payloads (2026-09-10)

Expanded `lx200 invalid coordinates` to 48 invalid Sr/Sd fixtures covering field widths, signs, separators, trailing/incomplete input, oversized integers, non-finite/exponent/hex forms and hour/degree/minute/second ranges. Each rejection must return 0, preserve prior RA/DEC when a subsequent MS publishes the target, and allow the next query; valid setters still work afterward. New `lx200 coordinate boundaries` tests six coordinate pairs with zero, pole and upper-RA boundaries, signed subdegree declinations, fractional seconds and the Meade-documented short RA decimal minutes. Both cases failed before the fix.

Validation: invalid coordinates, coordinate boundaries, truncated command, command length, ACK, positive/negative zero, full LX200 protocol and input matrix passed (8/8), including cleanup. The real agent/test executable built for macOS arm64/x86_64; scripted, hardware-free tests ran on arm64. The protocol reference is the bundled `indigo_drivers/mount_lx200/Meade-2010.10.pdf`. Version remains 0x03000016 as requested. Test artifacts removed with `make -C indigo_test test-clean`.

### DRV-141 — LX200 listener startup result (2026-09-10)
...
### DRV-144 — abort active processes before client teardown (2026-09-10)

Expanded `shutdown active` from one slew into nine operation/shutdown/reinitialization cycles: slew, sync, park, unpark, home and four dome operations. Mount, dome and rotator are selected with slaving/derotation enabled. Each shutdown has a five-second watchdog and must forward exactly one abort to each selected peer, return and permit reinitialization with no BUSY process. The final idle shutdown must not send additional aborts. The pre-fix test terminated with SIGALRM; all cycles pass after aborting and joining handlers while the internal client remains attached.

Validation: `shutdown active`, `shutdown server`, `coupled abort`, legacy/modern abort and success matrices, `lx200 idle stop`, `lx200 bind failure`, `LX200 protocol`, `feature persistence` and `persistent features` passed (12/12), including cleanup. Agent/test executable built for macOS arm64/x86_64; scripted, hardware-free tests ran on arm64. Physical motion cessation is not established by the fake-peer abort checks. Version remains 0x03000016 per user instruction. Test artifacts removed with `make -C indigo_test test-clean`.

### DRV-145 — finish empty start requests without BUSY (2026-09-10)

Expanded `all false start` covers a false single-item request at idle and after a missing-device error; repeated full all-false START vectors with peers selected; exactly one OK update and no peer operation commands; successful sync after each empty request; and preservation of a running slew when a false request arrives during BUSY. The held slew still completes normally. The regression failed before the fix and passes with the empty-selection early completion.

Validation: `all false start`, missing devices/capabilities, legacy/modern success and abort matrices, active/server shutdown and both persistence cases passed (11/11), including cleanup. Production agent/test executable built for macOS arm64/x86_64; scripted tests executed on arm64 without hardware/sockets. Version remains 0x03000016 per user instruction. Test artifacts removed with `make -C indigo_test test-clean`.

### DRV-145 extension — Imager, Guider and shared platesolver (2026-09-10)

Added `empty start` to Imager and Guider suites: repeated complete all-false vectors are accepted without devices, with camera only and with focuser/guider selected; exactly one OK update is published and camera request counts do not change. Valid single preview still completes, and an empty request cannot clear continuous preview while BUSY; explicit abort remains functional. Both regressions failed before the corresponding driver fix.

Added `platesolver empty start` in the Imager executable using a minimal attached device/client and the actual common platesolver/filter APIs used by ASTAP/Astrometry. It checks empty vectors without and with a related Imager, unchanged camera requests, and explicit SOLVE dispatch without an image source. No external solver process/index is required. After correcting fixture enumeration, a separate temporary executable linked with the original platesolver object reproduced the pre-fix failure; the current library passes.

Validation: Mount empty-start regression (1); Imager empty-start/platesolver regressions plus finite acquisitions, capture abort/reacquire and settings/reset (5); Guider empty-start, single preview, preview abort/restart, BUSY guards and reset defaults (5): 11/11 cases passed. Framework and Imager/Guider/ASTAP/Astrometry builds passed for macOS arm64/x86_64; execution was on arm64 with the CCD simulator. Driver versions remain unchanged per user instruction. Test build artifacts and temporary baseline executable/object/source were removed. Finding remains DRV-145 in `indigo_drivers/REVIEW.md`.

### DRV-146 — invalidate deleted mount/dome capabilities (2026-09-10)

Expanded `stale capability` covers all advertised mount/dome feature flags: individual HOME/PARK/TRACK and dome PARK/OPEN deletion, ALERT without issuing a peer operation, and successful operation after redefinition; mount coordinate-mode deletion disables SLEW/SYNC and redefinition restores them; both orders of deleting and restoring the dome coordinate-mode/horizontal-coordinate properties retain SLEW whenever either source remains. Deleting an unselected peer's property leaves selected mount capabilities intact.

Validation: `stale capability`, missing devices/capabilities, legacy/modern success, all three dome reselection cases, process deselection, legacy dome slew, shutdown active and all false start passed (12/12), including cleanup. Production agent and test executable built for macOS arm64/x86_64; scripted tests ran on arm64 without hardware or sockets. Version remains unchanged per user instruction. Test artifacts removed with `make -C indigo_test test-clean`.

### DRV-147 — wait for unpark before slew/sync (2026-09-10)
...
## 2026-09-12 — Atik generated CCD/guider/wheel fake SDK acceptance

Added `integration/test_ccd_atik_sdk.c` and `test-ccd-atik-sdk` to normal integration targets. Production driver is a separate compiled object with fake Artemis/libusb boundaries and real framework queues/countdown/image processing. All 36 named scenario groups passed together in final normal and ASan/UBSan arm64 runs, followed by targeted metadata assertion checks; prebuilt framework/dependency archives and LeakSanitizer are excluded. Configuration storage is isolated in a temporary directory.

Scenario-to-test mapping, individual groups, non-applicable class rows and evidence are recorded in `../indigo_drivers/ccd_atik/REFACTOR.md`. Coverage includes all three logical interfaces, six shared connection permutations, rollback/discovery/capacity, X_ properties and optional controls, RAW16 noise/ROI/asymmetric bins/383 crop, all frame types and fractional timing, abort/timeouts/races, cooling/heater/preset/gain/offset errors, CONFIG SDK restore, independent guide axes/80 pulse measurements and wheel motion/errors/reconnect. SDK instrumentation checks per-handle serialization, bus-thread calls, after-close calls, USB/lock balance and after-detach updates.

Non-applicable: driver streaming, Bayer conversion, suffix/name writes, image callbacks, serial protocol, calibration, wheel direction/speed, focuser/rotator and custom persistence. Generic framework ranges/codecs/upload containers and slot-name storage are not retested. Physical Atik Titan, One, 11000A and Horizon acceptance is pending; `hardware/test_ccd_atik_hw.c` is compiled, opt-in only and excluded from normal targets. Linux/Windows runtime, vendor internal timing/failures and electrical ST4/optical image/shutter behavior are not established by the fake.
...
### Independent QHY drivers and modern production hot-plug (2026-09-12)

Both variants now compile their own generated C++ source against their own SDK headers. The common fake SDK harness selects only its SDK ABI with QHY2; production drivers need no QHY2 define. Modern `discovery lifecycle` replaces the startup-only case with disconnected/idle/exposure/streaming USB removal, reattachment and fresh exposure. `discovery capacity identity` reverses USB order relative to SDK IDs, checks incomplete/error inventory does not detach cameras, removes the absent SDK identity, recovers available capacity and ignores already-known IDs. `init enumeration attachment` checks callback registration rollback for modern and inventory failure for legacy. Other acquisition, mode/depth, guider, CFW, settings, lifecycle and failure tests remain applicable. No simulator protocol changes: the SDK boundary is the simulator; existing physical evidence covers QHY5III178 and QHY5LII-M with modern SDK 26.7.21.5. Multiple physical cameras and other platforms remain unverified.

Validation: 37/37 cases passed for each independent variant (74/74 total); legacy x86_64/Rosetta and modern native arm64. Both production drivers and standalone executables built; regenerated outputs match.

## 2026-09-12 — Titan hardware shutter capability regression

Titan hardware exposed INVALID_FUNCTION from `ArtemisSetDarkMode` on a camera without a mechanical shutter. Driver version 33 uses `ArtemisProperties.cameraflags & ARTEMIS_PROPERTIES_CAMERAFLAGS_HAS_SHUTTER`. New `shutterless_acquisition` tests all five frame types, no unsupported setter calls, and capability refresh after reconnect; `acquisition_errors` still covers setter failures for a shutter-equipped camera and `durations_and_frame_types` covers dark/light mapping. Normal run: all behavior groups passed; the sole stale version assertion was updated and `metadata_profiles` passed separately (37 groups total). Logs `/tmp/atik-titan-fake-full.log` and `/tmp/atik-titan-fake-metadata.log`.

Opt-in hardware client adds `frame_types` with restoration, immediate fresh-ALERT failure, optional property-state diagnostics and RAW validation for all feature scenarios. Six Titan scenarios passed; exact timing, geometry, scope and pending hardware gaps are in `../TESTING.md` and `../indigo_drivers/ccd_atik/REFACTOR.md`. Standard fake SDK acceptance remains hardware-free. No temperature/cooler, gain/offset, heater, presets or wheel was exposed on this Titan; other models and electrical/optical measurement remain pending. The later physical-removal results are recorded below.

Targeted arm64 ASan/UBSan `shutterless_acquisition`, `acquisition_errors` and `durations_and_frame_types` all passed (`/tmp/atik-titan-asan-{shutter,errors,types}.log`). Driver, test and isolated framework driver object were instrumented; prebuilt dependencies and LeakSanitizer were excluded.

Physical follow-up: `hotplug` and `hotplug_active` are opt-in interactive cases only, with bounded presence waits, guider-first reconnection, a fresh image and guide command. Idle detach succeeded, idle replug crashed in an SDK thread; a separate active-exposure removal/replug cycle passed. A narrower SDK-only comparison did not reproduce the crash. See TESTING.md for exact reports/logs; fake coverage cannot establish vendor-internal lifetime safety. Reliable idle replug remains an unresolved hardware acceptance gap.

The `hotplug_idle` opt-in selector reuses only the idle phase, allowing the failed physical scenario to be repeated without another active-exposure cycle. One user-requested repeat passed with driver 33; the original crash remains unresolved (details in REFACTOR.md and TESTING.md). No production change for this repeat.


## 2026-09-12 — Atik One hardware-driven regressions

Driver version 36 adds three groups to the existing 37-group fake SDK suite: `abort_readout_restart` (SDK stop readout tail, bounded restart/timeout, no temperature read during the tail, reconnect recovery), `initial_wheel_unknown_target` (target 209 on connection, no automatic motion, explicit recovery and invalid in-flight target), and `wheel_wraparound_position` (moving current==count retains the last valid public slot, completion, stationary invalid index rejection and recovery). These scenarios come from real One failures; ordinary integration targets remain hardware-free. All 40 groups passed together against version 36 (`/tmp/atik-one-fake-v36-full.log`); generated C/header/main were byte-identical to a fresh generator run and scoped whitespace checks passed. Further evidence is recorded in ccd_atik/REFACTOR.md.

The explicitly approved shared-framework minimum 2 MiB handler queue stack has a real queued volatile 1 MiB write/read regression, `queue_supports_large_sdk_stack_frame`; all 90 timer/queue tests passed. Larger platform defaults remain unchanged. Targeted ASan/UBSan passed the three new Atik cases and existing stop-failure recovery, with the established prebuilt-library and LeakSanitizer exclusions.

The opt-in hardware client now tests all wheel slots and restoration, plus cooling convergence/power, a cooled image, cooler OFF and original target/selection restoration. One passed exposure/reconnect/reload, all advertised bins/ROI, read modes, five frame types, abort/restart, cooling and all five wheel positions including wraparound. One exposes no guider/heater/presets/gain/offset in this run. Physical One hot-plug subsequently passed as recorded below; 11000A/Horizon, electrical/optical accuracy and full warm-up measurement remain unverified; see REFACTOR.md and TESTING.md for evidence. No README changes.


Atik One hot-plug follow-up: the opt-in hardware client now includes any integrated wheel in presence checks, connects it before removal and reconnects/verifies its original slot after each replug. One combined idle/active-integration USB run passed with version 36: both interfaces detached/reattached, fresh .1 s RAW16 frames and wheel checks passed after each cycle, restoration and shutdown passed (exit 0). Evidence: `/tmp/atik-one-hw-v36-hotplug.log`; detailed times/limits in TESTING.md and ccd_atik/REFACTOR.md. This is real hardware acceptance only; no new fake scenario or production change was needed. Harness build and scoped whitespace checks passed; test-clean completed.

## 2026-09-12 — Atik2 generated CCD/guider/wheel acceptance

Migrated macOS `ccd_atik2` version 7 to generated INDIGO 3 driver version 9 and added `integration/test_ccd_atik2_sdk.c` to the normal integration target. Its 24 scenario groups compile production code separately against fake `libatik` and USB boundaries while retaining the real bus, property classes, handler queues, timers, CCD countdown and image processing. Coverage includes disconnect and USB removal during a blocked pixel readout, replug/recovery and wheel BUSY overlap rejection. The opt-in Atik hardware client is exposed through `test-ccd-atik2-hw` and remains excluded from normal tests.

All 24 independent scenario groups pass in the normal universal test build and full arm64 ASan/UBSan run. Coverage includes all six CCD/guider/wheel connection orders, optional capability profiles, open/lock/initialization rollback, validated SDK context data, all advertised bin modes, aligned ROI and deterministic RAW16 payload, short/long/fractional exposures, five frame types, bias minimum, read mode, start/read/dimension/abort/deadline failures, queued-start abort, disconnect and USB removal during blocked readout, countdown progress with an occupied queue, cooling and polling errors/cancellation, all four guide directions and independent-axis replacement, wheel initialization/move/BUSY rejection/poll/timeout/reconnect, the legacy false setter result and zero moving sentinel, duplicate/capacity/wrong-VID/descriptor and partial-attach hot-plug paths, active removal/replug, callback/queue failures and repeated INIT/SHUTDOWN. Every fixture asserts balanced USB references and global locks, no SDK call after close, no SDK work on the bus thread, no overlapping SDK entry and no property update after detach.

The final physical Atik One run passed valid RAW16 `.001/.1/1.5/2.5/16.5` second exposures, all five frame types, ROI, every advertised bin 1–128, both read modes, cooling with nonzero power and a cooled image, wheel slots 1–5–1, abort/restart, disconnect/reconnect, shutdown and `dlclose/dlopen` followed by a fresh exposure. The test restored wheel slot 1, cooler selection, temperature target and camera properties before clean shutdown. The hardware run exposed and fixed the bundled `libatik` 1.9 wheel return-value and moving-sentinel quirks; it also exposed a harness-only FITS/RAW restoration error. Guider/electrical timing, physical hot-plug for this driver, optical/shutter/cooling accuracy, multiple cameras, other models and non-macOS platforms remain gaps; see `../indigo_mac_drivers/ccd_atik2/REFACTOR.md` and `../TESTING.md`.
